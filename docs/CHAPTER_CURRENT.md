...
1. Add optional compatibility wrapper from old matrix entry to current baseline mode.
2. Decide which legacy verification scripts are archive targets for CH-02.
3. Start runtime split phase-1:
   - extract entry-funnel/report serialization from `src/runtime/BacktestRuntime.cpp`.
4. Add deterministic reproducibility check:
   - run identical `fixed` command twice and assert same gate-critical fields.
5. Start engine/strategy root-cause deep dive based on diagnostics:
...
6. Start adaptive lifecycle risk rewrite:
