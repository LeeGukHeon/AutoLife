...
61. Candidate-generation root-cause decomposition report 강화 + A/B 플레이북 자동화 (2026-02-20):
   - objective:
     - `candidate_generation` 병목을 단일 카운트가 아닌 컴포넌트/원인/실험축으로 분해해 다음 구조개선 실행성을 높임.
   - changes:
     - `scripts/run_verification.py`
       - per-dataset:
         - `diagnostics.per_dataset[*].candidate_generation_breakdown` 추가
         - 구성:
           - components: `no_signal_generated`, `filtered_out_by_manager`, `filtered_out_by_policy`, `no_best_signal`
           - component_shares + primary_component
           - no-signal / manager-prefilter / policy-prefilter reason breakdown(top + raw map)
           - shadow policy lift absolute/per-signal
           - A/B 실험 후보(`ab_playbook_candidates`)
       - aggregate:
         - `diagnostics.aggregate.candidate_generation_analysis` 추가
         - 구성:
           - aggregate components/shares + primary component
           - top no-signal reasons
           - top manager/policy prefilter reasons
           - aggregate A/B 실험 후보
       - failure attribution 확장:
         - `primary_candidate_generation_component`
         - `primary_candidate_generation_component_share`
         - `primary_no_signal_reason`
         - `primary_manager_prefilter_reason`
       - CLI 출력 확장:
         - `[Verification] primary_candidate_generation_component=... share=...`
   - validation:
     - `python scripts/verify_baseline.py --output-tag candidate_breakdown_v2` PASS
     - observed:
       - `primary_candidate_generation_component=no_signal_generated`
       - `share=0.9772`
       - top no-signal reason: `foundation_no_signal_liquidity_volume_gate`
       - aggregate A/B playbook emitted (`A_signal_supply`)
62. A_signal_supply 실험 경로(기본 OFF) 연결 + A/B 실행 (2026-02-20):
   - objective:
     - `foundation_no_signal_liquidity_volume_gate` 과다 구간에서 제한적 후보공급 fallback 경로를 실험 플래그로 검증.
   - changes:
     - `src/strategy/FoundationAdaptiveStrategy.cpp`
       - env flag 기반 실험 경로 추가:
         - `AUTOLIFE_ENABLE_EXPERIMENT_A_SIGNAL_SUPPLY=1`
       - `RANGING` 구간 한정 fallback path 추가:
         - `foundation_adaptive_regime_entry_signal_supply_fallback`
       - 기본값은 OFF (플래그 없으면 기존 로직 100% 동일).
     - `scripts/run_verification.py`
       - `--enable-experiment-a-signal-supply` 추가
       - 백테스트 실행 시 env 주입 + 리포트 `experiments` 필드 기록
     - `scripts/verify_baseline.py`
       - wrapper 옵션 `--enable-experiment-a-signal-supply` 추가
   - validation:
     - control:
       - `python scripts/verify_baseline.py --output-tag expA_off_control`
     - treatment:
       - `python scripts/verify_baseline.py --enable-experiment-a-signal-supply --output-tag expA_on_treatment_v2`
   - observed:
     - no-signal supply 변화:
       - `candidate_generation.no_signal_generated: 66706 -> 66254` (개선)
       - `filtered_out_by_manager: 1559 -> 1845` (manager prefilter로 이동)
     - 성능/게이트:
       - `avg_profit_factor`, `avg_expectancy_krw`, `avg_total_trades` 실질 변화 없음
       - baseline contract: `fail` (`primary_candidate_conversion_non_degrade_pass` 미통과)
   - decision:
     - 실험 경로는 유지하되 기본 OFF 유지
     - 다음 구조개선 우선순위는 `B_manager_prefilter` (소프트 큐/2단계 prefilter)로 전환
63. B_manager_prefilter 소프트 큐 실험 경로(기본 OFF) 연결 + 조합실험 (2026-02-20):
   - objective:
     - A 실험에서 매니저 prefilter로 이동한 near-miss 후보를 RANGING 한정 소프트 큐로 재평가.
   - changes:
     - `src/strategy/StrategyManager.cpp`
       - env flag 기반 실험 경로 추가:
         - `AUTOLIFE_ENABLE_EXPERIMENT_B_MANAGER_SOFT_QUEUE=1`
       - manager filter fail 중 일부 near-miss를 조건부 승격:
         - regime=`RANGING` 한정
         - reject_reason: `filtered_out_by_manager_ev_quality_floor/strength` 계열
         - strength/EV gap bounded + liquidity/volatility/RR guard
       - 기본 OFF(플래그 없으면 기존 경로 동일)
     - `scripts/run_verification.py`
       - `--enable-experiment-b-manager-soft-queue` 추가
       - 리포트 `experiments.enable_experiment_b_manager_soft_queue` 기록
     - `scripts/verify_baseline.py`
       - wrapper 옵션 `--enable-experiment-b-manager-soft-queue` 추가
   - validation (6-set):
     - control:
       - `python scripts/verify_baseline.py --output-tag expAB_off_control_v1`
     - A only:
       - `python scripts/verify_baseline.py --enable-experiment-a-signal-supply --output-tag expAB_a_only_v1`
     - B only:
       - `python scripts/verify_baseline.py --enable-experiment-b-manager-soft-queue --output-tag expAB_b_only_v1`
     - A+B:
       - `python scripts/verify_baseline.py --enable-experiment-a-signal-supply --enable-experiment-b-manager-soft-queue --output-tag expAB_ab_combo_v1`
   - observed:
     - B only:
       - `filtered_out_by_manager: 1559 -> 1371` (완화 확인)
       - `avg_primary_candidate_conversion: 0.7394 -> 0.7800`
       - PnL/거래수 변화 없음 (`avg_pf/avg_exp/avg_total_trades` 동일)
     - A+B:
       - `no_signal_generated: 66706 -> 66254` + manager 완화 동시 반영
       - PnL/거래수 변화 없음
     - baseline contract:
       - A only: fail (`primary_candidate_conversion_non_degrade_pass`)
       - B only / A+B: fail (`shadow_supply_lift_non_degrade_pass`)
   - decision:
     - A/B 실험 경로 모두 기본 OFF 유지
     - 다음 우선순위는 “후보 공급 증가를 실제 체결 품질로 연결하는 구조” (policy/selection 단계 개선)로 이동

64. Experiment C (adaptive liquidity gate) was tested and rolled back (2026-02-20):
   - objective:
     - verify whether additional low-liquidity signal supply can improve opportunity conversion.
   - implementation (temporary):
     - added `AUTOLIFE_ENABLE_EXPERIMENT_C_ADAPTIVE_LIQUIDITY_GATE` path in `FoundationAdaptiveStrategy`.
     - wired `--enable-experiment-c-adaptive-liquidity-gate` into verification scripts.
   - validation:
     - `python scripts/verify_baseline.py --enable-experiment-c-adaptive-liquidity-gate --output-tag expC_c_only_v1`
     - `python scripts/verify_baseline.py --enable-experiment-c-adaptive-liquidity-gate --output-tag expC_c_only_v2`
     - `python scripts/verify_baseline.py --enable-experiment-c-adaptive-liquidity-gate --output-tag expC_c_only_v3`
   - observed:
     - broad C path increased candidate supply but degraded quality (`blocked_risk_gate_entry_quality_edge_base` rise) and failed baseline contract.
     - tightened C path removed degradation but produced no meaningful improvement vs OFF.
   - decision:
     - removed C path from strategy and verification CLI to keep codebase lean.
     - baseline remains on OFF + A/B-only experimental framework.

65. Live liquidity-score scale mismatch fixed (2026-02-20):
   - issue:
     - `scanAllMarkets()` and `calculateLiquidityScore()` used different notional denominators (100B vs 1B KRW), causing inconsistent live liquidity scoring.
   - changes:
     - `src/analytics/MarketScanner.cpp`
       - introduced `kLiquidityScoreReferenceNotionalKrw = 100000000000.0`
       - unified both code paths to the same denominator.
   - validation:
     - build: `cmake --build build --config Release --target AutoLifeTrading -j 1` (vcpkg cmake path)
     - verify: `python scripts/verify_baseline.py --output-tag post_revert_c_marketscanner_fix_v1` PASS (baseline contract pass).

66. Probabilistic-hybrid transition kickoff (2026-02-20):
   - objective:
     - transition from threshold-heavy tuning loop to a hybrid path:
       - probabilistic pattern model (`P(up/down | pattern, context, indicators)`)
       - existing adaptive runtime risk/execution controls.
   - changes:
     - added long-horizon bundle collector:
       - `scripts/fetch_probabilistic_training_bundle.py`
     - features:
       - sequential fetch orchestration (market x timeframe), no parallel API burst
       - configurable 5 major + 5 legacy-alt universe
       - upbit-epoch window support (`2017-10-24T00:00:00Z` default start)
       - pre-fetch storage estimate (raw OHLCV vs indicator-enriched CSV)
       - post-fetch manifest/summary artifacts
   - artifacts:
     - manifest(default): `data/backtest_probabilistic/probabilistic_bundle_manifest.json`
     - summary(default): `build/Release/logs/probabilistic_bundle_summary.json`
   - runbook command:
     - estimate only:
       - `python scripts/fetch_probabilistic_training_bundle.py --estimate-only`
     - real fetch (sequential):
       - `python scripts/fetch_probabilistic_training_bundle.py --skip-existing`
   - validation:
     - estimate dry-run executed:
       - `python scripts/fetch_probabilistic_training_bundle.py --estimate-only`
       - summary:
         - `job_count=50` (10 markets x 5 TF)
         - estimated raw OHLCV CSV: `5.78 GB`
         - estimated indicator-enriched CSV (30 cols assumed): `29.41 GB`
     - real-fetch smoke executed:
       - `python scripts/fetch_probabilistic_training_bundle.py --markets-major KRW-BTC --markets-alt KRW-ETC --timeframes 240 --max-jobs 1 --skip-existing --sleep-ms-between-jobs 0`
       - result:
         - `KRW-BTC 240m`: `18152 rows`, `1.25 MB`
         - coverage: `2017-11-08` ~ `2026-02-20`
     - robustness fix:
       - `scripts/fetch_upbit_historical_candles.py`
         - fixed null `timestamp` parsing fallback to `candle_date_time_utc`.

67. Leakage-safe walk-forward split manifest generator added (2026-02-20):
   - objective:
     - enforce time-ordered split contract before probabilistic model training.
   - changes:
     - added split manifest generator:
       - `scripts/generate_probabilistic_split_manifest.py`
     - added model contract seed:
   ...
   - validation:
     ...
     - current smoke state:
       - `dataset_count=1` (KRW-BTC 240m sample fetched)
       - 3-fold anchored walk-forward windows generated.

## Remaining In This Chapter
1. Phase-1 data foundation for probabilistic-hybrid path (new top priority):
   - execute bundle estimate and freeze training universe/timeframes in artifact manifest.
   - run sequential real fetch for 10 markets x (1m,5m,15m,60m,240m) with resume (`--skip-existing`).
   - verify each fetched dataset has consistent monotonic timestamps and expected coverage windows.
2. Build probabilistic model input schema (before model training):
   - define leakage-safe feature snapshot timestamping (`t` features, `t+1..t+k` labels only).
   - define label family:
     - directional up/down probability
     - cost-aware expected-edge class/regression target.
   - produce model-ready dataset manifest and split policy (time-ordered walk-forward).
...
4. Continue adaptive lifecycle risk rewrite (phase-1 done):
   - phase-1 done:
