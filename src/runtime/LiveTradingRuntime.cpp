...
#include "common/TickSizeHelper.h"  // [Phase 3] ???????????? ???????????좎럡??????????��??????????????
...
void TradingEngine::scanMarkets() {
    ...
    for (const auto& coin : scanned_markets_) {
        ...
        LOG_INFO(
            "  #{} {} - margin {:+.4f}, 24h vol {:.0f}��, volatility {:.2f}%",
...
bool TradingEngine::executeBuyOrder(
    ...
    LOG_INFO("?????좎럩???????????????�얘???????? {} (??????????좎럥???????????좎룞??????????????좎럡?�?��??? {:.2f})", market, signal.strength);
    
    try {
        // 1. [????? ?????Orderbook) ???????????????????좎럩????????????????좎럥????????????????????????????좎럥??????????
        //    ?????????????좎럡??�땟戮녹???좎럩?????�?�????????????????????????좎듅?(Ticker)??????????? ????????좎럡??�땟戮녹???좎럩?????�?�???? ??????????????????좎뜴�???????????????????'??????좎럥???????????????�엨??????1???'????????????�㈇??????
        ...
        if (orderbook.is_array() && !orderbook.empty()) {
            ...
            LOG_ERROR("{} - ??? ?????????orderbook_units)???????좎럩???????????????????�싲�?��?????????????좎럥�?��?? {}", market, orderbook.dump());
            return false;
        }
        ...
        double best_ask_price = calculateOptimalBuyPrice(market, signal.entry_price, orderbook); // ??????좎럥???????????????�엨??????1???
        ...
        // [??????좎럥????????????????좎럥??�뜝?濚밸Þ??????????좎럥?????????????1] ?????????????????????????????좎럥???????????????????????????��????????????(???? ????????????????????????좎럩?????????
        // ????????�곣뫖利?�뜝???? signal.entry_amount????????????????????????????????????????????????????????????????汝뷴???좎룞??????좎럥????????????좎럩???�윻?????
        // ????????좎럡??�땟戮녹???좎럩?????�?�?????????? ???????????좎룞???????????????????????????좎럥???????? ???????��????????????????????좎럡??�땟戮녹???좎럩?????�?�??
        if (metrics.available_capital < invest_amount) {
             // ?????????????濚밸Ŧ???????? ??? ????????좎럡??�땟戮녹???좎럩?????�?�??????????筌롫???????? ????????좎럡??�땟戮녹???좎럩?????�?�?????????좎럥�?��?????????????�곣뫖利?�뜝??좎룞??�뜝???
             // ??????좎럥????????????????????�곣뫖利?�뜝?????좎럥?????????????좎럩?�▼???????????좎럥踰▼??????????????????좎럩???????饔낅??????????????????????�곣뫖利?�뜝??좎룞??�뜝??????????좎럩??�뜝??좎룞????좎럩??????
             if (metrics.available_capital < MIN_ORDER_BUFFER) {
                 LOG_WARN("{} - ????????좎룞???????????????????좎럩�????{:.0f} < {:.0f} (?????좎럩????????????????????좎럩�??)", 
                          market, metrics.available_capital, MIN_ORDER_BUFFER);
                 return false;
             }
             // ??? ???????????????????좎럡???????????좎럥????????????????????좎럥踰▼?????????????????????좎럥�??????? ??? ???????????좎룞????????????????????좎럥??????????(Partial Entry)
             LOG_INFO("{} - ????????좎룞???????????????????좎럩�????????????????????????????????????????? {:.0f} -> {:.0f}", 
                      market, invest_amount, metrics.available_capital);
             invest_amount = metrics.available_capital;
        }
        
        LOG_INFO("{} - [???????????? ????????????좎럩????????????좎럩?�욕????????? {:.0f} KRW (????????좎룞??????{:.0f})", 
                     market, invest_amount, metrics.available_capital);
        
        if (invest_amount < MIN_ORDER_BUFFER) {
            LOG_WARN("{} - ?????좎럩??????????????????좎럥�???????????좎럩�???????????좎럩?�욕????????? {:.0f}, ???????�곣뫖利?�뜝??좎룞??�뜝?? {:.0f} KRW) [??????????????]", 
                     market, invest_amount, MIN_ORDER_BUFFER);
            return false;
        }
        
        // [?????????????좎럥??????????좎럡??????? ?????????좎럡???????position_size??RiskManager???????
        if (!risk_manager_->canEnterPosition(
            market,
            signal.entry_price,
            safe_position_size,
            ...
            LOG_WARN("{} - ????????�뵁??????濚밸Ŧ?�욃????????�맩鍮??????????????????좎럥�??(????????�뵁??????濚밸Ŧ?�욃????????????", market);
            return false;
        }
        ...
        if (isLiveRealOrderEnabled()) {
            ...
            // [NEW] OrderManager?????????????????좎듅??????????�곣뫖利?�뜝?????좎럥?????????????좎럩?�▼??????????좎럩?????????
            // Strategy Metadata ????????좎럡??�땟戮녹???좎럩?????�?�???
            ...
            if (submitted) {
                LOG_INFO("OrderManager ???????꾩룆�????�붾굝?????????�챶�????????�얘???????????????�곣뫖利?�뜝??좎룞??�뜝?? {}", market);
                risk_manager_->reservePendingCapital(invest_amount);  // ???????좎럩?????????????????????????
            ...
            // Paper Trading (??????좎럥???????????????좎럥???????? - ???????????????????븍툖????????? (Simulation)
        ...
        LOG_ERROR("executeBuyOrder ????????좎럩?????? {}", e.what());
        return false;
    }
}
...
void TradingEngine::monitorPositions() {
    ...
    // 1. ????????좎럡??�땟戮녹???좎럩?????�?�????????????좎럥�??????????????좎럥??????????좎럡??????????????????좎럥???????????????좎럥?????????좎럥?�뜹????좎뜾????�?????좎럩?????????????????????????��??????????�붾굝?????????좎떊?곷퉲????좎럥?????????
    ...
    // 2. ?????????좎럡?????? ??????????좎럥??????????좎럡????????????????�싲�?��??????????????????????좎룞??�뜝??????좎뜾???????????좎럡??�땟戮녹???좎럩?????�?�????????좎럥???????????????????좎럥�????????(Batch ????????????????????좎럡?볟뜝??????????????????????
    ...
    if (should_log) {
        LOG_INFO("===== ??????????좎럩?????????????좎럩?????????????좎럡????({}????????繹먮�??? =====", markets.size());
    }

    // 3. [????? ?????????HTTP ?????????????????????좎럥???????????????좎럥???????????????�싲�?��????????????좎럡??�땟戮녹???좎럩?????�?�????????????????????????좎듅? ?????????????(Batch Processing)
    std::map<std::string, double> price_map;
    
    try {
        // MarketScanner???????????????汝뷴???좎룞????좎뜦�?��???getTickerBatch ?????
        ...
        LOG_ERROR("????????????????????????????????????????좎럥�?? {}", e.what());
        return; // ??????????????????????????????좎럥踰▼????????????? ??????좎럥??????????????? (?????????????좎럥???????????????�엨?????????????�곣뫖利?�뜝??좎룞??�뜝?????좎럥�???�????좎럡?볟젆??)
    }

    // 4. ??????좎럥?????????????????�곣뫖利?�뜝??좎룞??�뜝?????????????????????????? ??????????????????????????????????좎럥�?????좎럡?????(??????????????????? ??????좎럥?????????????????????룰�??�???
    for (auto& pos : positions) {
        ...
        // RiskManager ???????????????????????좎룞????????????�維뽳�??뱀????좎뜴????(??????좎럥?????????????????�곣뫖利?�뜝??좎룞??�뜝????????????饔낅???????
        risk_manager_->updatePosition(pos.market, current_price);
        
        // [????????????좎룞�?????????????????濚밸Ŧ?????? ????????좎럡??�땟戮녹???좎럩?????�?�??????"????????????좎룞????????????�維뽳�??뱀????좎뜴??????" ??????????????????좎럥???????????�얘??????????�곣�??????===================
        std::shared_ptr<strategy::IStrategy> strategy;
        if (strategy_manager_) {
            // ?????????????????좎럥�??????????????????嶺뚮?�維?????????좎럥�?��??????????????좎럡??�땟戮녹???좎럩?????�?�????????좎럥??????????????????????(pos.strategy_name ????
            strategy = strategy_manager_->getStrategy(pos.strategy_name);
            
            if (strategy) {
                // ????????????IStrategy????????????좎룞�?????updateState ???????��?????????
                // ???????????좎룞??????????????좎럡??�땟戮녹???좎럩?????�?�????????????좎럩?????????????????????????????좎럩???????饔낅????????????????????좎룞???????????룸㎗??�き????????좎럥????????????????????????좎럥�?????????????????????좎럥??????????????????????
                // ????????�얘?????????????????????좎럡??�땟戮녹???좎럩?????�?�????????????좎럩????????????????????좎룞???????????????????????????좎듅?????????????좎럥?????????????????????????좎럥?????(??????????????????좎뜴�??????
        ...
        if (should_log) {
            LOG_INFO("  {} - ???????�곣뫖利?�뜝??좎룞??�뜝?? {} - ?????좎럩????????? {:.0f}, ???????�곣뫖利?�뜝??좎룞??�뜝?? {:.0f}, ????? {:.0f} ({:+.2f}%)",
        ...
        if (strategy) {
            ...
            if (strategy->shouldExit(pos.market, updated_pos->entry_price, current_price, holding_time_seconds)) {
                LOG_INFO("???????�곣뫖利?�뜝??좎룞??�뜝????????????????????????黎앸??????沃섃???????? {} (???????�곣뫖利?�뜝??좎룞??�뜝?? {})", pos.market, updated_pos->strategy_name);
        ...
        if (!updated_pos->half_closed && current_price >= updated_pos->take_profit_1) {
            LOG_INFO("1?????????????????????????黎앸??????沃섃???????? (???????좎럥??�뜝???좎룞??????{:+.2f}%)", updated_pos->unrealized_pnl_pct * 100.0);
        ...
        if (risk_manager_->shouldExitPosition(pos.market)) {
            ...
            if (current_price <= updated_pos->stop_loss) {
                ...
                LOG_INFO("???????????????????????黎앸??????沃섃????????(???????좎럥??�뜝???좎룞??????{:+.2f}%)", updated_pos->unrealized_pnl_pct * 100.0);
            } else if (current_price >= updated_pos->take_profit_2) {
                reason = "take_profit";
                LOG_INFO("2????????????좎럩??????????????좎럡???좎럡???? ??????????????????黎앸??????沃섃????????(???????좎럥??�뜝???좎룞??????{:+.2f}%)", updated_pos->unrealized_pnl_pct * 100.0);
            } else {
                reason = "strategy_exit"; // ????????좎럡??�땟戮녹???좎럩?????�?�??????????????????????�?�???????????좎럡??????(TS ??
            }
        ...
        for (const auto& strategy : strategies) {
            ...
            for (const auto& request : order_requests) {
                ...
                if (order_amount < config_.min_order_krw) {
                    LOG_WARN("????????좎럥?????????????꾩룆�????�붾굝?????????�챶�?????????좎럩?�욕?????????????????좎럩�????{:.0f} < {:.0f}", order_amount, config_.min_order_krw);
                ...
                if (request.side == autolife::strategy::OrderSide::BUY) {
                    ...
                    if (reserved_capital <= 0.0 || order_amount > reserved_capital) {
                        LOG_WARN("{} ????????좎럥???????????좎럩??????????????좎럩????????좎럥??????�?��?????좎룞????? ????????????????????????좎럩�????({:.0f} < {:.0f})",
...
bool TradingEngine::executeSellOrder(
    ...
    LOG_INFO("???????�곣뫖利?�뜝??좎룞??�뜝???????좎럩????????????븍툖?????????????: {} (????: {})", market, reason);
    
    //double current_price = getCurrentPrice(market);
    if (current_price <= 0) {
        LOG_ERROR("???????�곣뫖利?�뜝??좎룞??�뜝??????????좎럩�??????????좎듅? ???????????????????????좎럥�?? {}", market);
        return false;
    }
    ...
    // 1. ??????좎럥????????????????????�곣뫖利?�뜝?????좎럥?????????????좎럩?�▼????????????�즾????????????????좎럥?????????????(??????좎럥?????????????????????좎럥???????????? 5,000 KRW)
    if (invest_amount < config_.min_order_krw) {
        LOG_WARN("?????좎럩????????????븍툖????????????좎럩?�욕?????????????????좎럩�????{:.0f} < {:.0f} (?????좎럩???좎뜾???????????????좎럩???????????", invest_amount, config_.min_order_krw);
        return false;
    }
    
    // 2. ??????????????????좎럡????????std::to_string ?????????? ?????????stringstream ????(??????좎럥????????????????????좎럥??????????좎럡???????
        std::stringstream ss;
        ss << std::fixed << std::setprecision(8) << sell_quantity;
        std::string quantity_str = ss.str();

    // 2-1. ?????????좎럥????????????????????좎럥??????????????????좎럥???????????????�엨????????? ????????????(??????좎럥??????????? ??????좎럥???????????????�엨?????????????????????
    ...
    try {
        ...
        if (slippage_pct > dynamic_sell_slippage.max_slippage_pct) {
            LOG_WARN("{} ?????? ????? {:.3f}% > {:.3f}% (?????????좎럩????????좎럥??????�?��?????좎룞?????",
                     market, slippage_pct * 100.0, dynamic_sell_slippage.max_slippage_pct * 100.0);
            return false;
        }
        LOG_INFO("?????좎럩????????????븍툖???????? ?????좎럩???????????? {} (???????�곣뫖利?�뜝??좎룞??�뜝??????????좎럩�??????????좎듅?: {})", sell_price, current_price);
    } catch (const std::exception& e) {
        LOG_WARN("??? ???????????????????????좎럥�??(???????�곣뫖利?�뜝??좎룞??�뜝??????????좎럩�??????????좎듅? ????: {}", e.what());
        sell_price = current_price;
    }

    // 2. ??????????????????좎뜴�?????????????�곣뫖利?�뜝?????좎럥?????????????좎럩?�▼??????????? (??????좎럥??????????? ????????? ???????�곣뫖利?�뜝??좎룞??�뜝????좎룞????????
    ...
    // 4. [Phase 3] ???????濚밸Ŧ????????????좎럥???????????vs ????????좎럡??�땟戮녹???좎럩?????�?�????????좎럥?????????????????????????
    ...
    // 5. [??????????????좎럥�???????? StrategyManager???????????????좎럡??�땟戮녹???좎럩?????�?�?????????좎럥???????????????????????????????좎룞????????????�維뽳�??뱀????좎뜴????& ?????????????????????????????
    if (fully_closed && strategy_manager_ && !position.strategy_name.empty()) {
        // Position ?????????????????????좎럩??�뜝??좎룞????좎럩?????????????좎럡??�뜝?????좎뜫??????????strategy_name("Advanced Scalping" ??????????????????좎럡??�땟戮녹???좎럩?????�?�????????좎럥??????????????????????
...
bool TradingEngine::executePartialSell(const std::string& market, const risk::Position& position, double current_price) {
    ...
    LOG_INFO("  ?????좎럩???????????: {:.0f}, ??????????좎뜴�?????: {:.0f}, ????????좎럩�??????????????????????�곣뫖利?�뜝??좎룞??�뜝?? {:.8f}",
    ...
    if (config_.mode == TradingMode::LIVE) {
        ...
        if (!order_result.success || order_result.executed_volume <= 0.0) {
            LOG_ERROR("????????좎럩�?????????좎럩????????????븍툖??????????좎럩????????????????????좎럥�?? {}", order_result.error_message);
            return false;
        }

        LOG_INFO("????????좎럩�?????????좎럩????????????븍툖??????????좎럩??????????????좎럥????????????? ??? {:.0f} (?????{})",
...
double TradingEngine::calculateOptimalBuyPrice(
    ...
    if (units.is_array() && !units.empty()) {
        ...
        return common::roundUpToTickSize(ask);  // [Phase 3] ??? ???????????좎럡???????
    }
...
double TradingEngine::calculateOptimalSellPrice(
    ...
    if (units.is_array() && !units.empty()) {
        ...
        return common::roundDownToTickSize(bid);  // [Phase 3] ??? ???????????좎럡????????
    }
...
TradingEngine::OrderFillInfo TradingEngine::verifyOrderFill(
    ...
    try {
        ...
        LOG_WARN("???????꾩룆�????�붾굝?????????�챶�???????좎럩??????????????좎럥???????????????????????좎럥�?? {}", e.what());
    }
...
TradingEngine::LimitOrderResult TradingEngine::executeLimitBuyOrder(
    ...
    while (running_ && remaining > 0.00000001 && attempt_count < max_attempts) {
        ...
        LOG_INFO("?????좎럩????????????????꾩룆�????�붾굝?????????�챶�?????????�곣뫖利?�뜝??좎룞??�뜝????좎룞????????좎럩�??(UUID: {}, ????????좎룞??????{:.0f}, ?????? {})", uuid, entry_price, vol_str);
        ...
        // ????????좎럩?????????汝뷴???좎룞????좎뜦�?��???????룸㎗??�き???????????????? ????????�곣뫖利?�뜝?????좎럥?????????????좎럩?�▼????????????????????
        try {
            http_client_->cancelOrder(uuid);
            LOG_WARN("?????좎럩????????????????좎럩??????β�?��??좎뜽�?????룸ħ瑗???????10?? -> ???????꾩룆�????�붾굝?????????�챶�???????????????????");
        } catch (const std::exception& e) {
            LOG_WARN("?????좎럩????????????????꾩룆�????�붾굝?????????�챶�??????????????????좎럥�?? {}", e.what());
        }
...
TradingEngine::LimitOrderResult TradingEngine::executeLimitSellOrder(
    ...
    while (running_ && remaining > 0.00000001 && attempt_count < max_attempts) {
        ...
        std::string uuid = order_res["uuid"].get<std::string>();
        LOG_INFO("?????좎럩????????????븍툖????????????꾩룆�????�붾굝?????????�챶�?????????�곣뫖利?�뜝??좎룞??�뜝????좎룞????????좎럩�??(UUID: {}, ????????좎룞??????{:.0f}, ?????? {})", uuid, exit_price, vol_str);
        ...
        try {
            http_client_->cancelOrder(uuid);
            LOG_WARN("?????좎럩????????????븍툖????????????좎럩??????β�?��??좎뜽�?????룸ħ瑗???????10?? -> ???????꾩룆�????�붾굝?????????�챶�???????????????????");
        } catch (const std::exception& e) {
            LOG_WARN("?????좎럩????????????븍툖????????????꾩룆�????�붾굝?????????�챶�??????????????????좎럥�?? {}", e.what());
        }
...
//     LOG_INFO("????????좎럡??�땟戮녹???좎럩?????�?�?????????????嶺뚮?�維??????????�?�럵????濚밸Ŧ?�앭???????);
...
double TradingEngine::getCurrentPrice(const std::string& market) {
    try {
        ...
        LOG_ERROR("???????�곣뫖利?�뜝??좎룞??�뜝??????????좎럩�??????????좎듅? ???????????????????????좎럥�?? {} - {}", market, e.what());
        return 0;
    }
}
    ...
    try {
        ...
        for (const auto& acc : accounts) {
            ...
            if (persisted) {
                ...
                LOG_INFO("????????????좎럥踰▼????좎뜫????????????좎뜾??????????좎럩?????????: {} SL={:.0f} TP1={:.0f} TP2={:.0f} BE={:.0f} TS={:.0f}",
                ...
                LOG_WARN("????????????좎럥踰▼????좎뜫????????????좎뜾???????????????: {} SL={:.0f} TP1={:.0f} TP2={:.0f} (?????좎럩????????????????????????????좎럥�??",
            ...
            if (half_closed) {
                ...
                if (pos) {
                    ...
                    LOG_INFO("  ????????좎럩�???????????????????????????????????좎럥踰▼????좎뜫??????? {} (half_closed=true)", market);
                }
            }
        }
        
        if (!krw_found) {
            LOG_WARN("???????????????KRW????????좎룞???? ???????�싲�?��?????????????좎럥�?��?? (???????�ル???????????0????????????????좎럩??????");
        ...
        if (!pending_reconcile_positions_.empty()) {
            ...
            for (const auto& pos : pending_reconcile_positions_) {
                ...
                LOG_INFO("???????좎럩?????????븍툖??????????????좎럥????????????? {} (???????�곣뫖利?�뜝??좎룞??�뜝?? {}, ????? {:.0f})",
...
std::string TradingEngine::exportPrometheusMetrics() const {
    // Prometheus ???????��????????????????좎럥�?????????좎럥?????????????饔낅?????????????????????????꾩룆�?���???��???????
    // Grafana?? ???????????좎럩??????????좎럩????????????????????좎럡??�뜝??????????좎럥???????????????좎럥????????????????�벡??????????좎럥??????????
    ...
    // ??????좎럥??????????????????(??? ???????饔낅????????????????????좎룞�???)
    oss << "# HELP autolife_state AutoLife ??????좎럥????????????????????????????????????????��???????????n";
    oss << "# TYPE autolife_state gauge\n";

    // ???????????????좎럥�???????????饔낅??????????
    oss << "# HELP autolife_capital_total ???????(KRW)\n";
    oss << "# TYPE autolife_capital_total gauge\n";
    oss << "# HELP autolife_capital_available ?????????????????????????좎럡??�뇡???????????????????????????????????? KRW)\n";
    oss << "# TYPE autolife_capital_available gauge\n";
    oss << "# HELP autolife_capital_invested ??????????????좎럥??????????좎럡????????????????�즾????????????????????좎럩???????????????????????? KRW)\n";
    oss << "# TYPE autolife_capital_invested gauge\n";

    // ???????????????좎럥�???????????饔낅??????????
    oss << "# HELP autolife_pnl_realized ????????????????????좎럡??�땟戮녹???좎럩?????�?�?? KRW)\n";
    oss << "# TYPE autolife_pnl_realized gauge\n";
    oss << "# HELP autolife_pnl_unrealized ????????좎럩?????????汝뷴???좎룞????좎뜦�?��??????????????????좎럡??�땟戮녹???좎럩?????�?�??????????????, KRW)\n";
    oss << "# TYPE autolife_pnl_unrealized gauge\n";
    oss << "# HELP autolife_pnl_total ??????????????????????좎럩?????????汝뷴???좎룞????좎뜦�?��????? KRW)\n";
    oss << "# TYPE autolife_pnl_total gauge\n";
    oss << "# HELP autolife_pnl_total_pct ????????좎럡??�땟戮녹???좎럩?????�?�????????????????????좎럥�?????????%)\n";
    oss << "# TYPE autolife_pnl_total_pct gauge\n";

    // ?????????좎럥????????μ?�媛?�??좎럩???????????????좎럥�???????????饔낅??????????
    oss << "# HELP autolife_drawdown_max ??????좎럥??????????? ????????좎럡??�땟戮녹???좎럩?????�?�?????????????????????\n";
    oss << "# TYPE autolife_drawdown_max gauge\n";
    oss << "# HELP autolife_drawdown_current ????????좎럡??�땟戮녹???좎럩?????�?�?????????????????????\n";
    oss << "# TYPE autolife_drawdown_current gauge\n";

    // ???????????????좎럥�???????????饔낅??????????
    oss << "# HELP autolife_positions_active ????????좎럡??�땟戮녹???좎럩?????�?�???????????좎럡?????? ???????n";
    oss << "# TYPE autolife_positions_active gauge\n";
    oss << "# HELP autolife_positions_max ???????????????????�??�뤷?????????좎럥??????????? ???????n";
    oss << "# TYPE autolife_positions_max gauge\n";

    // ??????좎럥??????????????????????????饔낅??????????
    oss << "# HELP autolife_trades_total ????????좎럡??�땟戮녹???좎럩?????�?�????????좎럥????????????????n";
    oss << "# TYPE autolife_trades_total counter\n";
    oss << "# HELP autolife_trades_winning ????????좎럡??�땟戮녹???좎럩?????�?�???????????좎럥�??????????????좎럥????????????????n";
    oss << "# TYPE autolife_trades_winning counter\n";
    oss << "# HELP autolife_trades_losing ????????좎럡??�땟戮녹???좎럩?????�?�?????????????좎럥????????????????n";
    oss << "# TYPE autolife_trades_losing counter\n";

    // ??????????????????좎럥�?????좎룞??????????좎럥?????????????????饔낅??????????
    oss << "# HELP autolife_winrate ????????좎럩?�놅??곗춹??좎뜦????????????0~1)\n";
    oss << "# TYPE autolife_winrate gauge\n";
    oss << "# HELP autolife_profit_factor ?????????좎럥�???????????????Profit Factor)\n";
    oss << "# TYPE autolife_profit_factor gauge\n";
    oss << "# HELP autolife_sharpe_ratio ?????????????좎럥????????????????????????????좎럥�?????좎룞??????????좎럥???????????\n";
    oss << "# TYPE autolife_sharpe_ratio gauge\n";

    // ??????????????????????????饔낅??????????
    oss << "# HELP autolife_engine_running ???????????????? ???????????1=??????????0=???)\n";
    oss << "# TYPE autolife_engine_running gauge\n";
    oss << "# HELP autolife_engine_scans_total ?????????????????�얘????????????????????좎뜫??�뜝?n";
    oss << "# TYPE autolife_engine_scans_total counter\n";
    oss << "# HELP autolife_engine_signals_total ????????꾩룆�?���???��????????????????????�?�??????????좎럥?????????n";
    oss << "# TYPE autolife_engine_signals_total counter\n";

    // ???????????좎룞??????좎뜫???????�쎼??????????????????????????????饔낅??????????

    // ??????????????좎럥????????????????????좎럥?????????????饔낅???????????????饔낅??????????
    oss << "# HELP autolife_buy_orders_total ????????좎럡??�땟戮녹???좎럩?????�?�????????좎럥??????????????????�곣뫖利?�뜝?????좎럥?????????????좎럩?�▼?????n";
    oss << "# TYPE autolife_buy_orders_total counter\n";
    oss << "# HELP autolife_sell_orders_total ????????좎럡??�땟戮녹???좎럩?????�?�????????좎럥???????????????�엨??????????????�곣뫖利?�뜝?????좎럥?????????????좎럩?�▼?????n";
    oss << "# TYPE autolife_sell_orders_total counter\n";
    oss << "# HELP autolife_pnl_cumulative ????????좎럡??�땟戮녹???좎럩?????�?�????????????????????????????�싲�?��??????????????좎럥�?��?????? KRW)\n";
...
void TradingEngine::runPrometheusHttpServer(int port) {
    ...
    LOG_INFO("Prometheus ?????좎럩????????????꿔꺂????????????�얘??????????????????좎럡??�린?????좎뜫????????????????????�곣뫖利?�뜝??좎룞??�뜝??(http://localhost:{}/metrics)", port);
