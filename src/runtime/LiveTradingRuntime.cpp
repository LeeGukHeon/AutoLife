...
#include "common/TickSizeHelper.h"  // [Phase 3] ???????????? ?????????ㅻ깹???????븐뼐?????????????
...
void TradingEngine::scanMarkets() {
    ...
    for (const auto& coin : scanned_markets_) {
        ...
        LOG_INFO("  #{} {} - score {:.1f}, 24h vol {:.0f}???뜚, volatility {:.2f}%",
...
void TradingEngine::executeSignals() {
    ...
    for (auto& signal : execution_candidates) {
        ...
        const double entry_edge_gate_base = computeContextualEdgeGateFloor(signal, config_, min_expected_edge_gate);

        double adaptive_rr_gate = min_reward_risk_gate;
        double adaptive_edge_gate = entry_edge_gate_base;
        if (small_seed_mode) {
            ...
            if (signal.market_regime == analytics::MarketRegime::HIGH_VOLATILITY ||
                ...
                adaptive_edge_gate = std::max(adaptive_edge_gate, entry_edge_gate_base * 1.45);
            } else if (favorable_small_seed_signal) {
                adaptive_rr_gate = std::max(config_.min_reward_risk + 0.08, adaptive_rr_gate - 0.08);
                adaptive_edge_gate = std::max(entry_edge_gate_base * 1.10, adaptive_edge_gate * 0.88);
            }
        }
        ...
        if (alpha_head_fallback_candidate) {
            ...
            adaptive_edge_gate = std::max(
                std::max(0.00035, entry_edge_gate_base * 0.35),
        ...
        adaptive_edge_gate = std::clamp(adaptive_edge_gate, entry_edge_gate_base, entry_edge_gate_base + 0.0012);
        ...
        if (config_.enable_entry_quality_adaptive_relief) {
            const bool adaptive_only_tightened =
                ...
                (adaptive_edge_gate > (entry_edge_gate_base + 1e-12));
            const bool high_stress_regime =
                signal.market_regime == analytics::MarketRegime::HIGH_VOLATILITY ||
                signal.market_regime == analytics::MarketRegime::TRENDING_DOWN;
            ...
            if (adaptive_only_tightened && quality_ok) {
                ...
                const double relieved_edge_gate = std::max(entry_edge_gate_base, adaptive_edge_gate - edge_relax);
        ...
        if (!adaptive_relief_applied) {
            ...
            const bool rr_base_ok = reward_risk_ratio >= min_reward_risk_gate;
            const bool edge_base_failed = calibrated_expected_edge_pct < entry_edge_gate_base;
            if (thin_uptrend && rr_base_ok && edge_base_failed) {
                ...
                const bool strong_quality =
                    ...
                    signal.expected_value >= std::max(0.00030, config_.entry_quality_adaptive_relief_min_expected_value * 0.85);
                const bool rr_surplus = reward_risk_ratio >= (min_reward_risk_gate + 0.10);
                if (tiny_edge_gap && strong_quality && rr_surplus) {
                    adaptive_edge_gate = std::max(entry_edge_gate_base - 0.00008, calibrated_expected_edge_pct);
                    adaptive_relief_applied = true;
                }
            }
        }
...
bool TradingEngine::executeBuyOrder(
    ...
    LOG_INFO("??遺얘턁??????????饔낅떽?????? {} (????????㏉렱????????ル뭽??癲??壤굿熬곥굥??? {:.2f})", market, signal.strength);
    
    try {
        // 1. [????? ?????Orderbook) ????????????????遺얘턁????????????釉먮폁????????????????????????뀀맩鍮???癲????
        //    ???????????熬곣뫖利당춯??쎾퐲??????????щ첉??????????쎛(Ticker)??????????? ??????熬곣뫖利당춯??쎾퐲???? ???????濡?씀?濾??μ떜媛??????????????????'????釉먮폁???????????븍툖?????1???'??????????곸궔?????
        auto orderbook = http_client_->getOrderBook(market);
        if (orderbook.empty()) {
            LOG_ERROR("??? ?????Β??????????????쇰뮝?? {}", market);
            return false;
        }
        
        // [????????썼린?濾?????熬곥끇???? API ???????ル뵁???????????????????????노륭?????????濡?씀?濾???????釉먮폁????????????꿔꺂???猷명ц땻??
        nlohmann::json units;
        if (orderbook.is_array() && !orderbook.empty()) {
            ...
            LOG_ERROR("{} - ??? ?????????orderbook_units)????遺얘턁????????????????繹먮굞??????????怨몄）: {}", market, orderbook.dump());
            return false;
        }
        ...
        // [NEW] executeSignals???????????????????????븐뼐????????????????곷♧?????????????
        if (signal.entry_amount > 0.0) {
            invest_amount = signal.entry_amount;
            // ???? position_size ???????ш끽紐????(???????ル뵁??????濚밸Ŧ援욆????????釉먮폁????????? ???뀀맩鍮???癲??????釉먮폁???????????????節딆퐵??????
        ...
        // [????釉먮폁????????????怨뚮뼺?源껊???????釉먮폁???????????1] ???????????????????????????釉먮폁?????????????濾?????????븐뼐???????????(???? ?????????????????????遺얘턁???????
        // ???????꾩룆梨???? signal.entry_amount????????????????????????????????????????????????????????濡?씀?濾??β뼯援???釉랁닑????????怨쀫뎔?????
        // ??????熬곣뫖利당춯??쎾퐲?????????? ????????ロ깫????????????????????????釉먮폁?????? ?????븐뼐?????????????????熬곣뫖利당춯??쎾퐲??
        if (metrics.available_capital < invest_amount) {
             // ?????????????源녾텛?????? ??? ??????熬곣뫖利당춯??쎾퐲??????????멸괜?????? ??????熬곣뫖利당춯??쎾퐲???????猷몄굣???????????꾩룆梨띰쭕???
             // ????釉먮폁?????????????????꾩룆梨???耀붾굝????????⑤챶裕????????怨뺤른??????????????遺얘턁?????꿔꺂???????????????????꾩룆梨띰쭕????????嚥싲갭큔?댁쉩?????
             if (metrics.available_capital < MIN_ORDER_BUFFER) {
                 LOG_WARN("{} - ??????ル뭽???????????????됱삩????{:.0f} < {:.0f} (??遺얘턁????????????????レ쑓??)", 
                          market, metrics.available_capital, MIN_ORDER_BUFFER);
                 return false;
             }
             // ??? ???????濡?씀?濾????ㅼ굣野???????釉먮폁????????????????怨뺤른??????????????????ㅼ뒧???嫄??? ??? ????????롮쾸?椰???????????????釉먮폁????????(Partial Entry)
             LOG_INFO("{} - ??????ル뭽???????????????됱삩???????Β????????????????嶺???????Β?????? {:.0f} -> {:.0f}", 
                      market, invest_amount, metrics.available_capital);
             invest_amount = metrics.available_capital;
        }
        
        LOG_INFO("{} - [???????????? ????????關?쒎첎?嫄???????援쏂굜???????? {:.0f} KRW (??????ル뭽????{:.0f})", 
                     market, invest_amount, metrics.available_capital);
        
        if (invest_amount < MIN_ORDER_BUFFER) {
            LOG_WARN("{} - ??遺얘턁??????????????怨뺤떪????????됱삩?????????援쏂굜???????? {:.0f}, ??????꾩룆梨띰쭕?? {:.0f} KRW) [??????????????]", 
                     market, invest_amount, MIN_ORDER_BUFFER);
            return false;
        }
        
        // [???????????썼린?濾?????熬곥끇???? ???????ㅻ깹??????position_size??RiskManager???????
        if (!risk_manager_->canEnterPosition(
            market,
            signal.entry_price,
            safe_position_size,
        ...
        // ????????????????ㅻ깹?????(????????????븐뼐????????????????????釉먮폁????????????꿔꺂???猷명ц땻?????????怨쀫엥????????怨룐솲???????????????????꾩룆梨????
        // [?????? ?????????????????(sprintf ?????stringstream ????
        char buffer[64];
        // ??????? ??????8????????轅붽틓???????, ???????????紐?????????0 ???????怨?????????????????붺몭???????????熬곣뫖利당춯??쎾퐲??????????????ш끽紐???
        std::snprintf(buffer, sizeof(buffer), "%.8f", quantity); 
        std::string vol_str(buffer);
        
        LOG_INFO("  ??????諛몄カ??轅붽틓?????곸뒭???????熬곣벀嫄?癲ル슣??????????? ??? {:.0f}, ??????{}, ?????援쏂굜????????{:.0f}", 
                 best_ask_price, vol_str, invest_amount);

            // 3. [???????濡?씀?濾??μ떜媛???? ???????濡?씀?濾??μ떜媛????????釉먮폁???????????????꾩룆梨???耀붾굝????????⑤챶裕??(OrderManager ??????熬곣뫖利당춯??쎾퐲??傭?끆?????筌?
        ...
        if (isLiveRealOrderEnabled()) {
            ...
            // [NEW] OrderManager???????????????숇????????꾩룆梨???耀붾굝????????⑤챶裕??????遺얘턁???????
            // Strategy Metadata ??????熬곣뫖利당춯??쎾퐲???
            ...
            if (submitted) {
                LOG_INFO("OrderManager ??????諛몄カ??轅붽틓?????곸뒭?????饔낅떽????????????꾩룆梨띰쭕?? {}", market);
                risk_manager_->reservePendingCapital(invest_amount);  // ????遺얘턁???????????????????????
            ...
            // Paper Trading (????釉먮폁???????????釉먮폇?????? - ??????????????????붺몭???????? (Simulation)
            ...
            try {
                ...
                if (!candles_json.empty() && candles_json.is_array()) {
                    ...
                    if (!candles.empty()) {
                        ...
                        LOG_INFO("[PAPER] ?????μ떜媛?걫?筌뚮챶夷????????????????쎛 ???????????? {:.0f} ({:.2f}%)", 
                                 dynamic_stop_loss, (dynamic_stop_loss - best_ask_price) / best_ask_price * 100.0);
                    }
                }
            } catch (const std::exception& e) {
                LOG_WARN("[PAPER] ?????μ떜媛?걫?筌뚮챶夷????????????????쎛 ????????????????????쇰뮝?? ??????????-2.5%) ???? {}", e.what());
            }
        ...
        LOG_ERROR("executeBuyOrder ????關?쒎첎?嫄??? {}", e.what());
        return false;
    }
}
...
void TradingEngine::monitorPositions() {
    ...
    // 1. ??????熬곣뫖利당춯??쎾퐲??????????노듋????????????썼린?濾?????熬곥끇?????????????釉먮폁???????????釉먮폇?????썹땟戮?눀筌롢룗爰??⑸역??????????????????????븐뼐????????轅붽틓?????됰Ŋ異??뀀맩鍮???????
    ...
    // 2. ???????ㅻ깹????? ????????썼린?濾?????熬곥끇????????????繹먮굝??????????????????亦껋꼨???ш끽維????????熬곣뫖利당춯??쎾퐲??????釉먮폁???????????????ㅼ뒧???怨????(Batch ??????????????????野껊챶???????????????????癲?
    ...
    if (should_log) {
        LOG_INFO("===== ???????遺얘턁?????????遺얜??????????ㅻ깹???({}????????깅뼂?? =====", markets.size());
    }

    // 3. [????? ?????????HTTP ???????????????????釉먮폁???????????釉먮폇????????????繹먮굝?????????熬곣뫖利당춯??쎾퐲??????????щ첉??????????쎛 ?????????????(Batch Processing)
    std::map<std::string, double> price_map;
    
    try {
        // MarketScanner???????????????β뼯援?癒⑸쨬??getTickerBatch ?????
        ...
        LOG_ERROR("??????????????????????Β??????????????쇰뮝?? {}", e.what());
        return; // ????????????????????????????怨뺤름???????????? ????釉먮폁?怨?땡?塋??????? (???????????釉먮폁???????????븍툖???????????꾩룆梨띰쭕???녾낮?녔틦釉껊뼀?)
    }

    // 4. ????釉먮폁??????????????꾩룆梨띰쭕?????????????筌???????????? ????????????????????????????????猷멥럸??ャ걤????(???????????????泥?? ????釉먮폁??????????????鶯???룰퀬寃???
    for (auto& pos : positions) {
        ...
        // RiskManager ??????濾??????????????⑤８????????袁⑸즴筌?씛彛???(????釉먮폁??????????????꾩룆梨띰쭕????????????꿔꺂?????
        risk_manager_->updatePosition(pos.market, current_price);
        
        // [?????????ш끽紐?????????????????源녾텛???? ??????熬곣뫖利당춯??쎾퐲??????"??????????⑤８????????袁⑸즴筌?씛彛?????" ????????????????釉먮폁??????饔낅떽???????꾩렯?????===================
        std::shared_ptr<strategy::IStrategy> strategy;
        if (strategy_manager_) {
            // ???????????????ㅼ뒧???怨?????????????留⑶뜮??????猷몄굡???????????熬곣뫖利당춯??쎾퐲??????釉먮폁??????????椰?筌???????(pos.strategy_name ????
            strategy = strategy_manager_->getStrategy(pos.strategy_name);
            
            if (strategy) {
                // ????????????IStrategy?????????ш끽紐?????updateState ?????븐뼐????????
                // ????????ロ깫???????????熬곣뫖利당춯??쎾퐲??????????⑤슢堉??곕??????????????????筌??遺얘턁?????꿔꺂???????????????ロ깫?????????룸ħ瑗??????釉먮폁????????????怨???????쇰뮛???????????????????뀀맩鍮???癲????????Β??????
                // ?????饔낅떽??????癰??????????熬곣뫖利당춯??쎾퐲??????????⑤슢堉??곕????????????ロ깫????????????????????????ヂ??????????산뭐???????????嚥???癲?????쇨덧????(???????濡?씀?濾??μ떜媛?????
                strategy->updateState(pos.market, current_price);
            }
        }

        // ????????????????????????????????熬곣몿???????????????????븐뼐????????轅붽틓?????됰Ŋ異??뀀맩鍮???????(???????ㅼ뒧???怨???????????????????????????븐뼐???????????
        auto* updated_pos = risk_manager_->getPosition(pos.market);
        if (!updated_pos) continue;
        
        if (should_log) {
            LOG_INFO("  {} - ??????꾩룆梨띰쭕?? {} - ??遺얘턁??????? {:.0f}, ??????꾩룆梨띰쭕?? {:.0f}, ????? {:.0f} ({:+.2f}%)",
        ...
        if (strategy) {
            ...
            if (strategy->shouldExit(pos.market, updated_pos->entry_price, current_price, holding_time_seconds)) {
                LOG_INFO("??????꾩룆梨띰쭕???????????Β???????????棺堉?뤃??믡굦?????? {} (??????꾩룆梨띰쭕?? {})", pos.market, updated_pos->strategy_name);
        ...
        if (!updated_pos->half_closed && current_price >= updated_pos->take_profit_1) {
            LOG_INFO("1????????????Β???????????棺堉?뤃??믡굦?????? (?????怨뚮뼺?됰뗀????{:+.2f}%)", updated_pos->unrealized_pnl_pct * 100.0);
            executePartialSell(pos.market, *updated_pos, current_price);
            continue; // ???????源녾텛????????釉먮폁???????????븍툖??????????????濡?씀?濾???????????繹먮굝??????????
        }
        ...
        if (risk_manager_->shouldExitPosition(pos.market)) {
            ...
            if (current_price <= updated_pos->stop_loss) {
                reason = "stop_loss";
                LOG_INFO("??????????Β???????????棺堉?뤃??믡굦??????(?????怨뚮뼺?됰뗀????{:+.2f}%)", updated_pos->unrealized_pnl_pct * 100.0);
            } else if (current_price >= updated_pos->take_profit_2) {
                reason = "take_profit";
                LOG_INFO("2?????????遺얘턁??????傭???癰궽쇱굩?? ?????Β???????????棺堉?뤃??믡굦??????(?????怨뚮뼺?됰뗀????{:+.2f}%)", updated_pos->unrealized_pnl_pct * 100.0);
            } else {
                reason = "strategy_exit"; // ??????熬곣뫖利당춯??쎾퐲????????????????????곗졒?????????ㅻ깹?????(TS ??
            }
        ...
        for (const auto& strategy : strategies) {
            ...
            for (const auto& request : order_requests) {
                ...
                if (order_amount < config_.min_order_krw) {
                    LOG_WARN("??????ル뵁???????????諛몄カ??轅붽틓?????곸뒭???????援쏂굜??????????????됱삩????{:.0f} < {:.0f}", order_amount, config_.min_order_krw);
                ...
                if (request.side == autolife::strategy::OrderSide::BUY) {
                    ...
                    if (reserved_capital <= 0.0 || order_amount > reserved_capital) {
                        LOG_WARN("{} ??????ル뵁???????遺얘턁?????????遺얘턁????怨뚮옩????影?ろ뀮???? ???????숈??????????????됱삩????({:.0f} < {:.0f})",
...
bool TradingEngine::executeSellOrder(
    ...
    LOG_INFO("??????꾩룆梨띰쭕????遺얘턁?????????붺몭????????????: {} (????: {})", market, reason);
    
    //double current_price = getCurrentPrice(market);
    if (current_price <= 0) {
        LOG_ERROR("??????꾩룆梨띰쭕?????Β?뗫쑄????????쎛 ?????Β??????????????쇰뮝?? {}", market);
        return false;
    }
    ...
    if (invest_amount < config_.min_order_krw) {
        LOG_WARN("??遺얘턁?????????붺몭?????????援쏂굜??????????????됱삩????{:.0f} < {:.0f} (??遺얘턁筌?（?????????遺얘턁??????傭??", invest_amount, config_.min_order_krw);
        return false;
    }
    
    // 2. ????????????????ㅻ깹???????std::to_string ?????????? ?????????stringstream ????(????釉먮폁????????????????썼린?濾?????熬곥끇????
        std::stringstream ss;
        ss << std::fixed << std::setprecision(8) << sell_quantity;
        std::string quantity_str = ss.str();

    // 2-1. ???????釉먮폁????????????????釉먮폁??????????????釉먮폁???????????븍툖???????? ????????????(????釉먮폁????????? ????釉먮폁???????????븍툖???????????????????
    ...
    try {
        ...
        LOG_WARN("??? ?????Β??????????????쇰뮝??(??????꾩룆梨띰쭕?????Β?뗫쑄????????쎛 ????: {}", e.what());
        sell_price = current_price;
    }

    // 2. ???????濡?씀?濾??μ떜媛???????????꾩룆梨???耀붾굝????????⑤챶裕?????????? (????釉먮폁????????? ????????? ??????꾩룆梨띰쭕?뚢뵾???????
    ...
    if (config_.mode == TradingMode::LIVE) {
        if (config_.dry_run || !config_.allow_live_orders) {
            ...
            if (!order_result.success || order_result.executed_volume <= 0.0) {
                LOG_ERROR("??遺얘턁?????????붺몭??????遺얘턁????????????????쇰뮝?? {}", order_result.error_message);
                return false;
            }

            executed_price = order_result.executed_price;
            sell_quantity = order_result.executed_volume;  // [Phase 3] ???????濡?씀?濾??μ떜媛????????釉먮폁????????????????꾩룆梨띰쭕?뚢뵾???????
            
            // [Phase 3] ???????源녾텛????????釉먮폁?????????????????????
            double fill_ratio = order_result.executed_volume / position.quantity;
            if (fill_ratio < 0.999) {
                LOG_WARN("??????됱삩??????遺얘턁??????????????ル뭽???: {:.8f}/{:.8f} ({:.1f}%)",
                         order_result.executed_volume, position.quantity, fill_ratio * 100.0);
            }
            
            LOG_INFO("??遺얘턁?????????붺몭??????遺얘턁??????????耀붾굝?????????? ??? {:.0f} (?????{})",
    ...
    // 5. [????????????ㅼ뒧???怨???? StrategyManager?????????????熬곣뫖利당춯??쎾퐲???????釉먮폁???????????????????????????⑤８????????袁⑸즴筌?씛彛???& ?????傭?끆?????????????????泥??
    if (fully_closed && strategy_manager_ && !position.strategy_name.empty()) {
        // Position ???????????????????嚥싲갭큔?댁쉩??????????癰귙룗???癲ル슢????????strategy_name("Advanced Scalping" ????????????????熬곣뫖利당춯??쎾퐲??????釉먮폁??????????椰?筌???????
...
bool TradingEngine::executePartialSell(const std::string& market, const risk::Position& position, double current_price) {
    ...
    LOG_INFO("  ??遺얘턁?????????: {:.0f}, ???????μ떜媛?걫???: {:.0f}, ??????됱삩???????????щ쫫????????꾩룆梨띰쭕?? {:.8f}",
    ...
    if (config_.mode == TradingMode::LIVE) {
        ...
        if (!order_result.success || order_result.executed_volume <= 0.0) {
            LOG_ERROR("??????됱삩??????遺얘턁?????????붺몭??????遺얘턁????????????????쇰뮝?? {}", order_result.error_message);
            return false;
        }

        LOG_INFO("??????됱삩??????遺얘턁?????????붺몭??????遺얘턁??????????耀붾굝?????????? ??? {:.0f} (?????{})",
...
double TradingEngine::calculateOptimalBuyPrice(
    ...
    if (units.is_array() && !units.empty()) {
        ...
        return common::roundUpToTickSize(ask);  // [Phase 3] ??? ?????????ㅻ깹??????
    }
...
double TradingEngine::calculateOptimalSellPrice(
    ...
    if (units.is_array() && !units.empty()) {
        ...
        return common::roundDownToTickSize(bid);  // [Phase 3] ??? ?????????ㅻ깹???????
    }
...
TradingEngine::OrderFillInfo TradingEngine::verifyOrderFill(
    ...
    try {
        ...
        LOG_WARN("??????諛몄カ??轅붽틓?????곸뒭????遺얘턁??????????耀붾굝??????????????????쇰뮝?? {}", e.what());
    }
...
TradingEngine::LimitOrderResult TradingEngine::executeLimitBuyOrder(
    ...
    while (running_ && remaining > 0.00000001 && attempt_count < max_attempts) {
        ...
        LOG_INFO("??遺얘턁?????????????諛몄カ??轅붽틓?????곸뒭????????꾩룆梨띰쭕?뚢뵾??萸??⑥쥓萸?(UUID: {}, ??????ル뭽????{:.0f}, ?????? {})", uuid, entry_price, vol_str);

        // 10???????????筌믨껴逾?????釉먮폁??????????????븐뼐???????????(500ms * 20)
        ...
        // ??????怨쀫엥?????β뼯援?癒⑸쨬?????룸ħ瑗???????????????? ???????꾩룆梨???耀붾굝????????⑤챶裕???????????????????
        try {
            http_client_->cancelOrder(uuid);
            LOG_WARN("??遺얘턁????????????됰엨????濡ろ뜐揶????룸ℓ???????10?? -> ??????諛몄カ??轅붽틓?????곸뒭???????????????????");
        } catch (const std::exception& e) {
            LOG_WARN("??遺얘턁?????????????諛몄カ??轅붽틓?????곸뒭????????????????쇰뮝?? {}", e.what());
        }
...
TradingEngine::LimitOrderResult TradingEngine::executeLimitSellOrder(
    ...
    while (running_ && remaining > 0.00000001 && attempt_count < max_attempts) {
        ...
        std::string uuid = order_res["uuid"].get<std::string>();
        LOG_INFO("??遺얘턁?????????붺몭??????????諛몄カ??轅붽틓?????곸뒭????????꾩룆梨띰쭕?뚢뵾??萸??⑥쥓萸?(UUID: {}, ??????ル뭽????{:.0f}, ?????? {})", uuid, exit_price, vol_str);
        ...
        try {
            http_client_->cancelOrder(uuid);
            LOG_WARN("??遺얘턁?????????붺몭?????????됰엨????濡ろ뜐揶????룸ℓ???????10?? -> ??????諛몄カ??轅붽틓?????곸뒭???????????????????");
        } catch (const std::exception& e) {
            LOG_WARN("??遺얘턁?????????붺몭??????????諛몄カ??轅붽틓?????곸뒭????????????????쇰뮝?? {}", e.what());
        }
...
double TradingEngine::getCurrentPrice(const std::string& market) {
    try {
        ...
        LOG_ERROR("??????꾩룆梨띰쭕?????Β?뗫쑄????????쎛 ?????Β??????????????쇰뮝?? {} - {}", market, e.what());
        return 0;
    }
}
...
void TradingEngine::syncAccountState() {
    ...
    try {
        ...
        for (const auto& acc : accounts) {
            ...
            // 1. [???????ㅼ뒧???怨???? KRW(???????????? ????釉먮폁????????????꿔꺂???猷명ц땻????????????붺몭????????????ш끽紐???
            if (currency == "KRW") {
                double total_cash = balance + locked; // ???????????+ ??????怨쀫엥?????β뼯援?癒⑸쨬?????룸ħ瑗?????????????轅붽틓?????
                
                // RiskManager????????????ㅼ뒧???????????????濡?씀?濾??μ떜媛???????????????????????????????ル뵁?????
                risk_manager_->resetCapital(total_cash);
                // (2) [?????????ш끽紐???] ???????????????濡?씀?濾????????????筌??'??????癲ル슢???????????????????濡?씀?濾??μ떜媛???????????????????ㅻ깹?????
                config_.initial_capital = total_cash;
                krw_found = true;
                LOG_INFO("???????????????????????????? {:.0f} KRW (??????ル뭽????{:.0f})", total_cash, balance);
                continue; // ????????????????釉먮폁????????????꿔꺂???猷명ц땻???????遺얘턁???????????????ш끽紐??????????濡?씀?濾??????????
            }

            // 2. ???????ㅻ깹????? ??????袁⑸즴筌?씛彛????????釉먮폁????????????꿔꺂???猷명ц땻??(??????????????????붺몭????????)
            // ????釉먮폁??????????????袁⑸즴筌?씛彛???????????諛몃마嶺뚮??????(?? BTC -> KRW-BTC)
            std::string market = "KRW-" + currency;
            wallet_markets.insert(market);
            
            double avg_buy_price = std::stod(acc["avg_buy_price"].get<std::string>());
            
            // ????釉먮폁??????饔낅떽?????????????Dust) ????饔낅떽????????(????釉먮폁?怨?땡?塋???????????釉먮폁?????????????????꾩룆梨???耀붾굝????????⑤챶裕????????곷♧????????????????袁⑸즴筌?씛彛???
            if (balance * avg_buy_price < 5000) continue;

            // ???? RiskManager?????????濡?씀?濾??β뼯援???釉랁닑??????????蹂㏓???????癲??
            if (risk_manager_->getPosition(market) != nullptr) continue;

            LOG_INFO("?????????????怨뺤른???? ?????獄쏅챶留?????????諛몃마嶺뚮??????ル늉筌띿떣??癲?? {} (?????? {:.8f}, ???????: {:.0f})", 
                     market, balance, avg_buy_price);

            // [Phase 4] ??????????筌?????????亦껋꼨???ш끽維?????????遺얘턁??????????SL/TP ???????ㅻ깹???轅붽틓??????????遺얘턁???????
            ...
            if (persisted) {
                ...
                LOG_INFO("??????????怨뺤른?癲ル슢???????ш끽維??????遺얘턁???????: {} SL={:.0f} TP1={:.0f} TP2={:.0f} BE={:.0f} TS={:.0f}",
                         market, safe_stop_loss, tp1, tp2, be_trigger, trail_start);
            } else {
                // ??????????筌?????????????????????泥??????????????(????釉먮폁?????????????????? ??????????留⑶뜮???????곗뵰????源낅뼌??????釉먮폁????????
                ...
                LOG_WARN("??????????怨뺤른?癲ル슢???????ш끽維??????????????: {} SL={:.0f} TP1={:.0f} TP2={:.0f} (??遺얘턁????????????????????????살몖??",
            ...
            if (half_closed) {
                ...
                if (pos) {
                    ...
                    LOG_INFO("  ??????됱삩???????????щ쫫??????????嫄?????????怨뺤른?癲ル슢????? {} (half_closed=true)", market);
                }
            }
        }
        
        if (!krw_found) {
            LOG_WARN("???????????????KRW??????ル뭽?? ??????繹먮굞??????????怨몄）! (??????筌롫㈇????????0????????????關?쒎첎?嫄???");
        ...
        if (!pending_reconcile_positions_.empty()) {
            ...
            if (!missing_markets.empty()) {
                try {
                    ...
                    LOG_WARN("?????됰엨???????붺몭???????????耀붾굝??????????????????????Β??????????????쇰뮝?? {}", e.what());
                }
            }
...
void TradingEngine::loadState() {
    try {
        ...
        LOG_ERROR("??????嫄?????????怨뺤른?癲ル슢???????ш끽維????????????쇰뮝?? {}", e.what());
    }
}

// ===== [NEW] ????????롮쾸?椰?癲ル슢??㎖?밤뀋???????????????????거???⑤벚猿?????????????????(???????ㅻ깹???????????????????????????? =====
...
// ===== [NEW] ???????? ??????꾩룆梨띰쭕?뚢뵾???????????????????(Win Rate & Profit Factor ??????????????? =====

double TradingEngine::calculatePositionScaleMultiplier() {
    // ????????????????:
    // Win Rate >= 60% AND Profit Factor >= 1.5 ?????????? ???????濡?씀?濾????ㅿ폍筌??
    // 
    // ???????????꾩룆梨띰쭕?뚢뵾??????????뀀맩鍮???癲????
    // - WR < 45% || PF < 1.0: 0.5??(?????????????????????ш끽紐????
    // - 45% <= WR < 50% || 1.0 <= PF < 1.2: 0.75??(???????ㅻ깹?????
    // - 50% <= WR < 60% || 1.2 <= PF < 1.5: 1.0??(???)
    // - WR >= 60% && PF >= 1.5: 1.5??2.5??(???)
    
    auto metrics = risk_manager_->getRiskMetrics();
    
    // ????釉먮폁?怨?땡?塋???????????????????源녾텛??????????????????거??????????? ??????꾩룆梨띰쭕?뚢뵾??????????
    ...
    if (win_rate < 0.45 || profit_factor < 1.0) {
        ...
        // ?????????????????????????쇰뮛???ㅺ껀??????? ?????????????
        // PF?? WR????????????????????遺얘턁??????????????꾩룆梨띰쭕?뚢뵾??????????뀀맩鍮???癲????
        // WR 60%~75%: 1.5??2.0?? PF 1.5~2.5: ???????ш끽紐??? 0.25??
        ...
        new_multiplier = std::min(2.5, new_multiplier);  // ????釉먮폁????????? 2.5??
    }
...
void TradingEngine::learnOptimalFilterValue() {
    // historical P&L ???????????????????????????????????거???⑤벚猿?????????????????????????깅짆???믊삳룛???????????藥???
    // ?????????
    // 1. ????釉먮폁?怨?땡?塋???????????????signal_filter ??????????????????????????釉먮폁?怨?땡?塋????????????????藥???????
    // 2. ????????????????????거???⑤벚猿?????????????????????????????깅짆???믊삳룛?????釉먮폁????????????????????(Win Rate, Profit Factor, Sharpe Ratio)
    // 3. ????釉먮폁??????????????????????????쇰뮛???ㅺ껀????????????????????거???⑤벚猿????????????ш끽紐????
    ...
    for (const auto& trade : history) {
        // signal_filter???????????????????????????????????????0.01 ?????????ㅻ깹???????????꾩룆梨띰쭕?뚢뵾???????????援쏂굜?????
    ...
    for (auto& [filter_val, trades] : trades_by_filter) {
        ...
        for (const auto& trade : trades) {
            if (trade.profit_loss > 0) {
                ...
                total_loss += std::abs(trade.profit_loss);  // ??????釉랁닑??????????????????????????
            }
        }
    ...
    // 3. ???뀀맩鍮???癲??????????????藥????????????ш끽紐????
    // ???????ш끽紐??? ??????????????? Win Rate >= 50% ??Profit Factor >= 1.2 ??????????????(???????濡?씀?濾????
...
// ===== [NEW] Prometheus ????釉먮폁???????????꿔꺂???????????븐뼐????????=====

std::string TradingEngine::exportPrometheusMetrics() const {
    // Prometheus ?????븐뼐?????????????욱룏???????釉먮폁???????????꿔꺂??????????????????????諛몃마嶺뚮??????
    // Grafana?? ?????????쏙쭗??????遺얘턁????????????????熬곣몿????????釉먮폁???????????釉먮폇????????????곕춴???????釉먮폁????????
    ...
    // ????釉먮폁????????????????(??? ???????꿔꺂????癒?떴????????ш끽紐???)
    oss << "# HELP autolife_state AutoLife ????釉먮폁?怨?땡?塋????????????????????濾?????????븐뼐??????????n";
    oss << "# TYPE autolife_state gauge\n";

    // ?????????????노듋???????????꿔꺂????癒?떴?
    oss << "# HELP autolife_capital_total ???????(KRW)\n";
    oss << "# TYPE autolife_capital_total gauge\n";
    oss << "# HELP autolife_capital_available ?????????????????????轝?癰궽븐숯????????????????????????????????? KRW)\n";
    oss << "# TYPE autolife_capital_available gauge\n";
    oss << "# HELP autolife_capital_invested ????????????썼린?濾?????熬곥끇???????????곷♧????????????????遺얘턁?????????????????????? KRW)\n";
    oss << "# TYPE autolife_capital_invested gauge\n";

    // ?????????????노듋???????????꿔꺂????癒?떴?
    oss << "# HELP autolife_pnl_realized ??????????????????熬곣뫖利당춯??쎾퐲?? KRW)\n";
    oss << "# TYPE autolife_pnl_realized gauge\n";
    oss << "# HELP autolife_pnl_unrealized ??????怨쀫엥?????β뼯援?癒⑸쨬???????????????熬곣뫖利당춯??쎾퐲??????????????, KRW)\n";
    oss << "# TYPE autolife_pnl_unrealized gauge\n";
    oss << "# HELP autolife_pnl_total ????????????????????怨쀫엥?????β뼯援?癒⑸쨬???? KRW)\n";
    oss << "# TYPE autolife_pnl_total gauge\n";
    oss << "# HELP autolife_pnl_total_pct ??????熬곣뫖利당춯??쎾퐲??????????????????ㅼ뒧???怨?????%)\n";
    oss << "# TYPE autolife_pnl_total_pct gauge\n";

    // ???????ル뵁??????濚밸Ŧ援욆????????????노듋???????????꿔꺂????癒?떴?
    oss << "# HELP autolife_drawdown_max ????釉먮폁????????? ??????熬곣뫖利당춯??쎾퐲?????????????????????\n";
    oss << "# TYPE autolife_drawdown_max gauge\n";
    oss << "# HELP autolife_drawdown_current ??????熬곣뫖利당춯??쎾퐲?????????????????????\n";
    oss << "# TYPE autolife_drawdown_current gauge\n";

    // ?????????????노듋???????????꿔꺂????癒?떴?
    oss << "# HELP autolife_positions_active ??????熬곣뫖利당춯??쎾퐲?????????ㅻ깹????? ???????n";
    oss << "# TYPE autolife_positions_active gauge\n";
    oss << "# HELP autolife_positions_max ???????濡?씀?濾????ㅿ폍筌??????釉먮폁????????? ???????n";
    oss << "# TYPE autolife_positions_max gauge\n";

    // ????釉먮폁?怨?땡?塋??????????????????꿔꺂????癒?떴?
    oss << "# HELP autolife_trades_total ??????熬곣뫖利당춯??쎾퐲??????釉먮폁?怨?땡?塋????????n";
    oss << "# TYPE autolife_trades_total counter\n";
    oss << "# HELP autolife_trades_winning ??????熬곣뫖利당춯??쎾퐲?????????ㅼ뒧???怨????????釉먮폁?怨?땡?塋????????n";
    oss << "# TYPE autolife_trades_winning counter\n";
    oss << "# HELP autolife_trades_losing ??????熬곣뫖利당춯??쎾퐲???????????釉먮폁?怨?땡?塋????????n";
    oss << "# TYPE autolife_trades_losing counter\n";

    // ????????????????쇰뮛???ㅺ껀??????釉먮폁???????????????꿔꺂????癒?떴?
    oss << "# HELP autolife_winrate ??????獄쏅챶留덌┼???????????0~1)\n";
    oss << "# TYPE autolife_winrate gauge\n";
    oss << "# HELP autolife_profit_factor ???????ㅼ뒧???怨???????????Profit Factor)\n";
    oss << "# TYPE autolife_profit_factor gauge\n";
    oss << "# HELP autolife_sharpe_ratio ???????????釉먮폁????????????????????????쇰뮛???ㅺ껀??????釉먮폁?????????\n";
    oss << "# TYPE autolife_sharpe_ratio gauge\n";

    // ??????????????濾???????????꿔꺂????癒?떴?
    oss << "# HELP autolife_engine_running ???????????????? ??????濾????1=??????????0=???)\n";
    oss << "# TYPE autolife_engine_running gauge\n";
    oss << "# HELP autolife_engine_scans_total ??????????????饔낅떽????????????????⑸㎣簾?n";
    oss << "# TYPE autolife_engine_scans_total counter\n";
    oss << "# HELP autolife_engine_signals_total ???????諛몃마嶺뚮?????????????????곗졒????????遺븍き???????n";
    oss << "# TYPE autolife_engine_signals_total counter\n";

    // ????????롮쾸?椰?癲ル슢??㎖?밤뀋???????????????????????????꿔꺂????癒?떴?
    oss << "# HELP autolife_filter_value_dynamic ????????롮쾸?椰?癲ル슢??㎖?밤뀋???????????????????거???⑤벚猿?????(0~1)\n";
    oss << "# TYPE autolife_filter_value_dynamic gauge\n";
    oss << "# HELP autolife_position_scale_multiplier ???????? ??????꾩룆梨띰쭕?뚢뵾???????n";
    oss << "# TYPE autolife_position_scale_multiplier gauge\n";

    // ????????????釉먮폁?怨?땡?塋??????????釉먮폁???????????꿔꺂?????????????꿔꺂????癒?떴?
    oss << "# HELP autolife_buy_orders_total ??????熬곣뫖利당춯??쎾퐲??????釉먮폁???????????????꾩룆梨???耀붾굝????????⑤챶裕????n";
    oss << "# TYPE autolife_buy_orders_total counter\n";
    oss << "# HELP autolife_sell_orders_total ??????熬곣뫖利당춯??쎾퐲??????釉먮폁???????????븍툖????????????꾩룆梨???耀붾굝????????⑤챶裕????n";
    oss << "# TYPE autolife_sell_orders_total counter\n";
    oss << "# HELP autolife_pnl_cumulative ??????熬곣뫖利당춯??쎾퐲???????????????????????????繹먮굝???????????猷몄굣????? KRW)\n";
...
void TradingEngine::runPrometheusHttpServer(int port) {
    ...
    LOG_INFO("Prometheus ??遺얘턁?????????嶺뚮∥???????饔낅떽???????鶯??????熬곣벀嫄?癲ル슣?????????????????꾩룆梨띰쭕??(http://localhost:{}/metrics)", port);
