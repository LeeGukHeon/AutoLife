...
#include "common/TickSizeHelper.h"  // [Phase 3] ???????????? ???????????醫롫윞??????????거??????????????
...
void resetRangingShadowArtifactLive() {
    const auto path = rangingShadowArtifactPathLive();
    std::filesystem::create_directories(path.parent_path());
    std::ofstream out(path, std::ios::binary | std::ios::trunc);
}

void appendRangingShadowEventLive(const nlohmann::json& line) {
    const auto path = rangingShadowArtifactPathLive();
    std::filesystem::create_directories(path.parent_path());
...
std::string normalizeRegimeEntryDisableKey(autolife::analytics::MarketRegime regime) {
...
void TradingEngine::scanMarkets() {
    ...
    const auto& filtered = scanned_markets_;
    ...
    live_signal_funnel_.quality_sort_mode = "vnext_margin_rank";
    live_signal_funnel_.quality_selected_candidates = static_cast<long long>(scanned_markets_.size());
    ...
    live_signal_funnel_.quality_margin_valid_count = static_cast<long long>(margin_values.size());
    ...
    live_signal_funnel_.quality_margin_mean = compute_mean(margin_values);
    live_signal_funnel_.quality_margin_std =
        compute_std(margin_values, live_signal_funnel_.quality_margin_mean);
    live_signal_funnel_.quality_margin_p10 = compute_quantile(margin_values, 0.10);
    live_signal_funnel_.quality_margin_p50 = compute_quantile(margin_values, 0.50);
    live_signal_funnel_.quality_margin_p90 = compute_quantile(margin_values, 0.90);

    LOG_INFO(
        "TopK select(vnext_margin_rank): selected={}, margin_valid={}, margin_mean={:+.4f}, p10={:+.4f}, p50={:+.4f}, p90={:+.4f}",
        static_cast<int>(live_signal_funnel_.quality_selected_candidates),
        static_cast<int>(live_signal_funnel_.quality_margin_valid_count),
        live_signal_funnel_.quality_margin_mean,
        live_signal_funnel_.quality_margin_p10,
        live_signal_funnel_.quality_margin_p50,
        live_signal_funnel_.quality_margin_p90
    );
    LOG_INFO(
    ...
    for (const auto& coin : scanned_markets_) {
        ...
        LOG_INFO(
            "  #{} {} - margin {:+.4f}, 24h vol {:.0f}억, volatility {:.2f}%",
            count, coin.market, coin.model_margin_score,
...
void TradingEngine::generateSignals() {
    ...
    for (const auto& coin : scanned_markets_) {
        ...
        try {
            ...
            if (collect_diag.liq_vol_gate_observation_count > 0) {
                live_signal_funnel_.liq_vol_gate_observation_count +=
                    static_cast<long long>(collect_diag.liq_vol_gate_observation_count);
                live_signal_funnel_.liq_vol_gate_pass_count +=
                    static_cast<long long>(collect_diag.liq_vol_gate_pass_count);
                live_signal_funnel_.liq_vol_gate_fail_count +=
                    static_cast<long long>(collect_diag.liq_vol_gate_fail_count);
                live_signal_funnel_.liq_vol_gate_low_conf_triggered_count +=
                    static_cast<long long>(collect_diag.liq_vol_gate_low_conf_triggered_count);
                live_signal_funnel_.liq_vol_gate_observed_sum += collect_diag.liq_vol_gate_observed_sum;
                live_signal_funnel_.liq_vol_gate_threshold_sum += collect_diag.liq_vol_gate_threshold_sum;
                live_signal_funnel_.liq_vol_gate_mode = collect_diag.liq_vol_gate_mode;
                live_signal_funnel_.liq_vol_gate_quantile_q = collect_diag.liq_vol_gate_quantile_q;
                live_signal_funnel_.liq_vol_gate_window_minutes =
                    collect_diag.liq_vol_gate_window_minutes;
                live_signal_funnel_.liq_vol_gate_min_samples_required =
                    collect_diag.liq_vol_gate_min_samples_required;
                live_signal_funnel_.liq_vol_gate_low_conf_action =
                    collect_diag.liq_vol_gate_low_conf_action;
                for (const auto& sample : collect_diag.liq_vol_gate_samples) {
                    if (live_signal_funnel_.liq_vol_gate_samples.size() >= 10) {
                        break;
                    }
                    LiveSignalFunnelTelemetry::LiqVolGateSample out_sample;
                    out_sample.market = sample.market;
                    out_sample.observed = sample.observed;
                    out_sample.threshold_dynamic = sample.threshold_dynamic;
                    out_sample.history_count = sample.history_count;
                    out_sample.pass = sample.pass;
                    out_sample.low_conf = sample.low_conf;
                    live_signal_funnel_.liq_vol_gate_samples.push_back(std::move(out_sample));
                }
            }
            if (collect_diag.structure_gate_observation_count > 0) {
                live_signal_funnel_.structure_gate_observation_count +=
                    static_cast<long long>(collect_diag.structure_gate_observation_count);
                live_signal_funnel_.structure_gate_fail_count_total +=
                    static_cast<long long>(collect_diag.structure_gate_fail_count_total);
                for (const auto& kv : collect_diag.structure_gate_fail_count_by_regime) {
                    if (kv.first.empty() || kv.second <= 0) {
                        continue;
                    }
                    live_signal_funnel_.structure_gate_fail_count_by_regime[kv.first] +=
                        static_cast<long long>(kv.second);
                }
                for (const auto& kv : collect_diag.structure_gate_pass_count_by_regime) {
                    if (kv.first.empty() || kv.second <= 0) {
                        continue;
                    }
                    live_signal_funnel_.structure_gate_pass_count_by_regime[kv.first] +=
                        static_cast<long long>(kv.second);
                }
                live_signal_funnel_.structure_gate_mode = collect_diag.structure_gate_mode;
                live_signal_funnel_.structure_gate_relax_delta =
                    collect_diag.structure_gate_relax_delta;
                live_signal_funnel_.structure_gate_observed_score_sum +=
                    collect_diag.structure_gate_observed_score_sum;
                live_signal_funnel_.structure_gate_threshold_before_sum +=
                    collect_diag.structure_gate_threshold_before_sum;
                live_signal_funnel_.structure_gate_threshold_after_sum +=
                    collect_diag.structure_gate_threshold_after_sum;
                for (const auto& sample : collect_diag.structure_gate_samples) {
                    if (live_signal_funnel_.structure_gate_samples.size() >= 10) {
                        break;
                    }
                    LiveSignalFunnelTelemetry::StructureGateSample out_sample;
                    out_sample.market = sample.market;
                    out_sample.regime = sample.regime;
                    out_sample.observed_score = sample.observed_score;
                    out_sample.threshold_before = sample.threshold_before;
                    out_sample.threshold_after = sample.threshold_after;
                    out_sample.pass = sample.pass;
                    out_sample.relax_applied = sample.relax_applied;
                    live_signal_funnel_.structure_gate_samples.push_back(std::move(out_sample));
                }
            }
            if (collect_diag.bear_rebound_observation_count > 0) {
                live_signal_funnel_.bear_rebound_guard_observation_count +=
                    static_cast<long long>(collect_diag.bear_rebound_observation_count);
                live_signal_funnel_.bear_rebound_guard_pass_count +=
                    static_cast<long long>(collect_diag.bear_rebound_pass_count);
                live_signal_funnel_.bear_rebound_guard_fail_count +=
                    static_cast<long long>(collect_diag.bear_rebound_fail_count);
                live_signal_funnel_.bear_rebound_guard_low_conf_triggered_count +=
                    static_cast<long long>(collect_diag.bear_rebound_low_conf_triggered_count);
                for (const auto& kv : collect_diag.bear_rebound_fail_count_by_regime) {
                    if (kv.first.empty() || kv.second <= 0) {
                        continue;
                    }
                    live_signal_funnel_.bear_rebound_guard_fail_count_by_regime[kv.first] +=
                        static_cast<long long>(kv.second);
                }
                for (const auto& kv : collect_diag.bear_rebound_pass_count_by_regime) {
                    if (kv.first.empty() || kv.second <= 0) {
                        continue;
                    }
                    live_signal_funnel_.bear_rebound_guard_pass_count_by_regime[kv.first] +=
                        static_cast<long long>(kv.second);
                }
                live_signal_funnel_.bear_rebound_guard_mode = collect_diag.bear_rebound_mode;
                live_signal_funnel_.bear_rebound_guard_quantile_q =
                    collect_diag.bear_rebound_quantile_q;
                live_signal_funnel_.bear_rebound_guard_window_minutes =
                    collect_diag.bear_rebound_window_minutes;
                live_signal_funnel_.bear_rebound_guard_min_samples_required =
                    collect_diag.bear_rebound_min_samples_required;
                live_signal_funnel_.bear_rebound_guard_low_conf_action =
                    collect_diag.bear_rebound_low_conf_action;
                live_signal_funnel_.bear_rebound_guard_observed_sum +=
                    collect_diag.bear_rebound_observed_sum;
                live_signal_funnel_.bear_rebound_guard_threshold_sum +=
                    collect_diag.bear_rebound_threshold_sum;
                for (const auto& sample : collect_diag.bear_rebound_samples) {
                    if (live_signal_funnel_.bear_rebound_guard_samples.size() >= 10) {
                        break;
                    }
                    LiveSignalFunnelTelemetry::BearReboundGuardSample out_sample;
                    out_sample.market = sample.market;
                    out_sample.regime = sample.regime;
                    out_sample.observed = sample.observed;
                    out_sample.threshold_dynamic = sample.threshold_dynamic;
                    ...
                    out_sample.pass = sample.pass;
                    out_sample.low_conf = sample.low_conf;
                    live_signal_funnel_.bear_rebound_guard_samples.push_back(std::move(out_sample));
                }
            }
            bool signals_suppressed_by_regime_shadow = false;
            live_signal_funnel_.regime_entry_disable_enabled =
            ...
            if (!signals.empty() && probabilistic_snapshot.phase3_regime_entry_disable_enabled) {
                ...
                if (isRegimeEntryDisabled(
                    ...
                    for (const auto& shadow_signal : signals) {                        const auto veto_probe = probeRealtimeEntryVeto(
                            config_,
                            shadow_signal,
                            signal_metrics.orderbook_snapshot,
                            hostility_probe,
                            recent_best_ask_by_market_.count(signal_metrics.market) > 0
                                ? recent_best_ask_by_market_.at(signal_metrics.market)
                                : 0.0,
                            recent_best_ask_timestamp_by_market_.count(signal_metrics.market) > 0
                                ? recent_best_ask_timestamp_by_market_.at(signal_metrics.market)
                                : 0LL,
                            now_ms
                        );
                        ...
                        shadow_line["ts_ms"] = now_ms;
                        shadow_line["source"] = "live";
                        shadow_line["mode"] = "ranging_shadow";
                        shadow_line["market"] = signal_metrics.market;
                        shadow_line["regime"] = regime_key;
                        shadow_line["vol_bucket_pct"] = autolife::common::runtime_diag::volatilityBucket(
                            signal_metrics.volatility
                        );
                        ...
                        shadow_line["ev_blend"] = shadow_signal.phase3.adaptive_ev_blend;                        shadow_line["disabled_reason"] = "REGIME_RANGING_SHADOW";
                        shadow_line["would_pass_quality_selection"] = true;
                        shadow_line["would_pass_execution_guard"] = would_pass_execution_guard;
                        shadow_line["note"] = veto_probe.veto_reason;
                        shadow_line["entry_price_proxy_close_t"] = current_price;
                        shadow_line["future_horizon_bars"] = 5;
                        shadow_line["future_close_t5_available"] = false;
                        ...
                        appendRangingShadowEventLive(shadow_line);

                        live_signal_funnel_.shadow_count_total++;
                        live_signal_funnel_.shadow_count_by_regime[regime_key]++;
                        live_signal_funnel_.shadow_count_by_market[signal_metrics.market]++;
                        if (would_pass_execution_guard) {
                            live_signal_funnel_.shadow_would_pass_execution_guard_count++;
                        }
                        if (shadow_signal.phase3.expected_edge_calibrated_bps < 0.0) {
                            live_signal_funnel_.shadow_edge_neg_count++;
                        } else if (shadow_signal.phase3.expected_edge_calibrated_bps > 0.0) {
                            live_signal_funnel_.shadow_edge_pos_count++;
                        }
                    }

                    live_signal_funnel_.reject_regime_entry_disabled_count +=
                        static_cast<long long>(skipped_candidates);
                    live_signal_funnel_
                        .reject_regime_entry_disabled_by_regime[regime_key] +=
                        static_cast<long long>(skipped_candidates);
                    markScanReject("reject_regime_entry_disabled_count", skipped_candidates);
                    markScanReject("reject_regime_entry_disabled", skipped_candidates);
                    markScanReject(
                    ...
                    signals.clear();
                    markScanReject("reject_regime_ranging_shadow_mode", skipped_candidates);
                }
            }
...
void TradingEngine::flushLiveSignalFunnelTaxonomyReport(
    ...
    const double liq_vol_gate_pass_rate =
        snapshot.liq_vol_gate_observation_count > 0
            ? static_cast<double>(snapshot.liq_vol_gate_pass_count) /
                  static_cast<double>(snapshot.liq_vol_gate_observation_count)
            : 0.0;
    const double liq_vol_gate_observed_mean =
        snapshot.liq_vol_gate_observation_count > 0
            ? snapshot.liq_vol_gate_observed_sum /
                  static_cast<double>(snapshot.liq_vol_gate_observation_count)
            : 0.0;
    const double liq_vol_gate_threshold_mean =
        snapshot.liq_vol_gate_observation_count > 0
            ? snapshot.liq_vol_gate_threshold_sum /
                  static_cast<double>(snapshot.liq_vol_gate_observation_count)
            : 0.0;
    const double structure_gate_observed_score_mean =
        snapshot.structure_gate_observation_count > 0
            ? snapshot.structure_gate_observed_score_sum /
                  static_cast<double>(snapshot.structure_gate_observation_count)
            : 0.0;
    const double structure_gate_threshold_before_mean =
        snapshot.structure_gate_observation_count > 0
            ? snapshot.structure_gate_threshold_before_sum /
                  static_cast<double>(snapshot.structure_gate_observation_count)
            : 0.0;
    const double structure_gate_threshold_after_mean =
        snapshot.structure_gate_observation_count > 0
            ? snapshot.structure_gate_threshold_after_sum /
                  static_cast<double>(snapshot.structure_gate_observation_count)
            : 0.0;
    const double bear_rebound_guard_pass_rate =
        snapshot.bear_rebound_guard_observation_count > 0
            ? static_cast<double>(snapshot.bear_rebound_guard_pass_count) /
                  static_cast<double>(snapshot.bear_rebound_guard_observation_count)
            : 0.0;
    const double bear_rebound_guard_observed_mean =
        snapshot.bear_rebound_guard_observation_count > 0
            ? snapshot.bear_rebound_guard_observed_sum /
                  static_cast<double>(snapshot.bear_rebound_guard_observation_count)
            : 0.0;
    const double bear_rebound_guard_threshold_mean =
        snapshot.bear_rebound_guard_observation_count > 0
            ? snapshot.bear_rebound_guard_threshold_sum /
                  static_cast<double>(snapshot.bear_rebound_guard_observation_count)
            : 0.0;
    nlohmann::json liq_vol_gate_samples = nlohmann::json::array();
    for (const auto& sample : snapshot.liq_vol_gate_samples) {
        liq_vol_gate_samples.push_back({
            {"market", sample.market},
            {"observed", sample.observed},
            {"threshold_dynamic", sample.threshold_dynamic},
            {"history_count", sample.history_count},
            {"pass", sample.pass},
            {"low_conf", sample.low_conf},
        });
    }
    nlohmann::json structure_gate_samples = nlohmann::json::array();
    for (const auto& sample : snapshot.structure_gate_samples) {
        structure_gate_samples.push_back({
            {"market", sample.market},
            {"regime", sample.regime},
            {"observed_score", sample.observed_score},
            {"threshold_before", sample.threshold_before},
            {"threshold_after", sample.threshold_after},
            {"pass", sample.pass},
            {"relax_applied", sample.relax_applied},
        });
    }
    nlohmann::json bear_rebound_guard_samples = nlohmann::json::array();
    for (const auto& sample : snapshot.bear_rebound_guard_samples) {
        bear_rebound_guard_samples.push_back({
            {"market", sample.market},
            {"regime", sample.regime},
            {"observed", sample.observed},
            {"threshold_dynamic", sample.threshold_dynamic},
    ...
    report["reject_regime_entry_disabled_count"] = snapshot.reject_regime_entry_disabled_count;
    report["reject_regime_entry_disabled_by_regime"] =
        snapshot.reject_regime_entry_disabled_by_regime;
    report["regime_entry_disable_enabled"] = snapshot.regime_entry_disable_enabled;
    report["regime_entry_disable"] = snapshot.regime_entry_disable;
    report["shadow_count_total"] = snapshot.shadow_count_total;
    report["shadow_count_by_regime"] = snapshot.shadow_count_by_regime;
    report["shadow_count_by_market"] = snapshot.shadow_count_by_market;
    report["shadow_would_pass_execution_guard_count"] =
        snapshot.shadow_would_pass_execution_guard_count;
    report["shadow_edge_neg_count"] = snapshot.shadow_edge_neg_count;
    report["shadow_edge_pos_count"] = snapshot.shadow_edge_pos_count;
    report["liq_vol_gate_dynamic"] = {
        {"mode", snapshot.liq_vol_gate_mode},
        {"quantile_q", snapshot.liq_vol_gate_quantile_q},
        {"window_minutes", snapshot.liq_vol_gate_window_minutes},
        {"min_samples_required", snapshot.liq_vol_gate_min_samples_required},
        {"low_conf_action", snapshot.liq_vol_gate_low_conf_action},
        {"observation_count", snapshot.liq_vol_gate_observation_count},
        {"pass_count", snapshot.liq_vol_gate_pass_count},
        {"fail_count", snapshot.liq_vol_gate_fail_count},
        {"low_conf_triggered_count", snapshot.liq_vol_gate_low_conf_triggered_count},
        {"pass_rate", liq_vol_gate_pass_rate},
        {"observed_mean", liq_vol_gate_observed_mean},
        {"threshold_dynamic_mean", liq_vol_gate_threshold_mean},
        {"samples", liq_vol_gate_samples},
    };
    report["foundation_structure_gate"] = {
        {"mode", snapshot.structure_gate_mode},
        {"relax_delta", snapshot.structure_gate_relax_delta},
        {"observation_count", snapshot.structure_gate_observation_count},
        {"fail_count_total", snapshot.structure_gate_fail_count_total},
        {"fail_count_by_regime", snapshot.structure_gate_fail_count_by_regime},
        {"pass_count_by_regime", snapshot.structure_gate_pass_count_by_regime},
        {"observed_score_mean", structure_gate_observed_score_mean},
        {"threshold_before_mean", structure_gate_threshold_before_mean},
        {"threshold_after_mean", structure_gate_threshold_after_mean},
        {"samples", structure_gate_samples},
    };
    report["bear_rebound_guard"] = {
        {"mode", snapshot.bear_rebound_guard_mode},
        {"quantile_q", snapshot.bear_rebound_guard_quantile_q},
        {"window_minutes", snapshot.bear_rebound_guard_window_minutes},
        {"min_samples_required", snapshot.bear_rebound_guard_min_samples_required},
        {"low_conf_action", snapshot.bear_rebound_guard_low_conf_action},
        {"observation_count", snapshot.bear_rebound_guard_observation_count},
        {"pass_count", snapshot.bear_rebound_guard_pass_count},
        {"fail_count", snapshot.bear_rebound_guard_fail_count},
        {"low_conf_triggered_count", snapshot.bear_rebound_guard_low_conf_triggered_count},
        {"pass_count_by_regime", snapshot.bear_rebound_guard_pass_count_by_regime},
        {"fail_count_by_regime", snapshot.bear_rebound_guard_fail_count_by_regime},
        {"pass_rate", bear_rebound_guard_pass_rate},
    ...
    report["quality_sort_mode"] = snapshot.quality_sort_mode;
    report["quality_selected_candidates"] = snapshot.quality_selected_candidates;
    report["quality_margin_stats"] = {
        {"valid_count", snapshot.quality_margin_valid_count},
        {"mean", snapshot.quality_margin_mean},
        {"std", snapshot.quality_margin_std},
        {"p10", snapshot.quality_margin_p10},
        {"p50", snapshot.quality_margin_p50},
        {"p90", snapshot.quality_margin_p90},
    };
...
void TradingEngine::executeSignals() {
    ...
    const double stop_guard_min_pct = std::clamp(config_.entry_capacity_stop_guard_min_pct, 0.0, 1.0);
    const double stop_guard_max_pct = std::clamp(
        config_.entry_capacity_stop_guard_max_pct,
        stop_guard_min_pct,
        1.0
    );
    const double exit_retention_floor = std::clamp(config_.entry_capacity_exit_retention_floor, 0.01, 0.99);
    const double fee_rate_for_capacity = Config::getInstance().getFeeRate();
    const double capacity_stop_guard_pct = std::clamp(
        config_.entry_capacity_default_stop_guard_pct,
        stop_guard_min_pct,
        stop_guard_max_pct
    );
    const double capacity_slippage_guard_pct = std::clamp(
        config_.max_slippage_pct *
            std::max(0.0, config_.entry_capacity_slippage_guard_multiplier),
        0.0005,
        0.02
    );
    const double capacity_exit_retention = std::max(
        exit_retention_floor,
        1.0 - capacity_stop_guard_pct - capacity_slippage_guard_pct - fee_rate_for_capacity
    );
    ...
    for (auto& signal : execution_candidates) {
        ...
        autolife::common::signal_policy::normalizeSignalStopLossByRegime(signal, signal_regime);
        ...
        const double stop_guard_pct = std::clamp(
            (risk_pct > 1e-9) ? risk_pct : capacity_stop_guard_pct,
            stop_guard_min_pct,
            stop_guard_max_pct
        );
        const double exit_retention = std::max(
            exit_retention_floor,
            1.0 - stop_guard_pct - slippage_guard_pct - fee_rate
        );
...
bool TradingEngine::executeBuyOrder(
    ...
    LOG_INFO("?????醫롫윪???????????????븐뼐???????? {} (??????????醫롫윥???????????醫롫짗??????????????醫롫윞?袁?쐻??? {:.2f})", market, signal.strength);
    
    try {
        // 1. [????? ?????Orderbook) ???????????????????醫롫윪????????????????醫롫윥????????????????????????????醫롫윥??????????
        //    ?????????????醫롫윞??밸븶筌믩끃???醫롫윪?????醫?筌????????????????????????醫롫뱟?(Ticker)??????????? ????????醫롫윞??밸븶筌믩끃???醫롫윪?????醫?筌???? ??????????????????醫롫쑕筌???????????????????'??????醫롫윥???????????????곗뿨??????1???'????????????ㅳ늾??????
        auto orderbook = http_client_->getOrderBook(market);
        if (orderbook.empty()) {
            LOG_ERROR("??? ???????????????????????醫롫윥筌?? {}", market);
            return false;
        }
        
        // [??????????醫롫윥??????????醫롫윞??????? API ?????????醫롫윥??????????????????????????醫롫짗??????????????????????????醫롫윥??????????????耀붾굝???????醫롫윥筌?????
        nlohmann::json units;
        if (orderbook.is_array() && !orderbook.empty()) {
            ...
            LOG_ERROR("{} - ??? ?????????orderbook_units)???????醫롫윪???????????????????μ떜媛?걫?????????????醫롫윥筌?짘?? {}", market, orderbook.dump());
            return false;
        }
        ...
        double best_ask_price = calculateOptimalBuyPrice(market, signal.entry_price, orderbook); // ??????醫롫윥???????????????곗뿨??????1???
        ...
        const double stop_guard_pct = std::clamp(
            (signal_risk_pct > 1e-9) ? signal_risk_pct : capacity_stop_guard_pct,
            stop_guard_min_pct,
            stop_guard_max_pct
        );
        const double slippage_guard_pct = dynamic_buy_slippage.guard_slippage_pct;
        ...
        // [NEW] executeSignals?????????????????????????거???????????????????ㅼ┥??????????????
        if (signal.entry_amount > 0.0) {
            invest_amount = signal.entry_amount;
            // ???? position_size ??????????醫롫짗壤????(?????????醫롫윥????????關?쒎첎?嫄??醫롫윪???????????醫롫윥??????????? ?????醫롫윥??????????????醫롫윥?????????????????鶯ㅺ동???醫?筌??????
        ...
        // [??????醫롫윥????????????????醫롫윥??〓쐻?嚥싲갭횧??????????醫롫윥?????????????1] ?????????????????????????????醫롫윥???????????????????????????거????????????(???? ????????????????????????醫롫윪?????????
        // ????????ш끽維뽳쭩?룸쐻???? signal.entry_amount????????????????????????????????????????????????????????????????黎앸럽???醫롫짗??????醫롫윥????????????醫롫윪???レ쑜?????
        // ????????醫롫윞??밸븶筌믩끃???醫롫윪?????醫?筌?????????? ???????????醫롫짗???????????????????????????醫롫윥???????? ???????거????????????????????醫롫윞??밸븶筌믩끃???醫롫윪?????醫?筌??
        if (metrics.available_capital < invest_amount) {
             // ?????????????嚥싲갭큔???????? ??? ????????醫롫윞??밸븶筌믩끃???醫롫윪?????醫?筌??????????嶺뚮∥???????? ????????醫롫윞??밸븶筌믩끃???醫롫윪?????醫?筌?????????醫롫윥筌?짘?????????????ш끽維뽳쭩?룸쐻??醫롫짗??용쐻???
             // ??????醫롫윥????????????????????ш끽維뽳쭩?룸쐻?????醫롫윥?????????????醫롫윪?닳뼹???????????醫롫윥甕겸뼹??????????????????醫롫윪???????耀붾굝??????????????????????ш끽維뽳쭩?룸쐻??醫롫짗??용쐻??????????醫롫윪??롫쐻??醫롫짗????醫롫윪??????
             if (metrics.available_capital < MIN_ORDER_BUFFER) {
                 LOG_WARN("{} - ????????醫롫짗???????????????????醫롫윪亦????{:.0f} < {:.0f} (?????醫롫윪????????????????????醫롫윪筌??)", 
                          market, metrics.available_capital, MIN_ORDER_BUFFER);
                 return false;
             }
             // ??? ???????????????????醫롫윞???????????醫롫윥????????????????????醫롫윥甕겸뼹?????????????????????醫롫윥獒??????? ??? ???????????醫롫짗????????????????????醫롫윥??????????(Partial Entry)
             LOG_INFO("{} - ????????醫롫짗???????????????????醫롫윪亦????????????????????????????????????????? {:.0f} -> {:.0f}", 
                      market, invest_amount, metrics.available_capital);
             invest_amount = metrics.available_capital;
        }
        
        LOG_INFO("{} - [???????????? ????????????醫롫윪????????????醫롫윪?띠슃????????? {:.0f} KRW (????????醫롫짗??????{:.0f})", 
                     market, invest_amount, metrics.available_capital);
        
        if (invest_amount < MIN_ORDER_BUFFER) {
            LOG_WARN("{} - ?????醫롫윪??????????????????醫롫윥甕???????????醫롫윪亦???????????醫롫윪?띠슃????????? {:.0f}, ???????ш끽維뽳쭩?룸쐻??醫롫짗??용쐻?? {:.0f} KRW) [??????????????]", 
                     market, invest_amount, MIN_ORDER_BUFFER);
            return false;
        }
        
        // [?????????????醫롫윥??????????醫롫윞??????? ?????????醫롫윞???????position_size??RiskManager???????
        if (!risk_manager_->canEnterPosition(
            market,
            signal.entry_price,
            safe_position_size,
        ...
        if (slippage_pct > dynamic_buy_slippage.max_slippage_pct) {
            LOG_WARN("{} ?????? ????? {:.3f}% > {:.3f}% (?????醫롫윪??????????????醫롫윪????????醫롫윥??????傭?끆?????醫롫짗?????",
        ...
        // ??????????????????醫롫윞??????(??????????????거???????????????????????醫롫윥??????????????耀붾굝???????醫롫윥筌??????????????醫롫윪??????????????醫롫윥筌?????????????????????ш끽維뽳쭩?룸쐻????
        // [?????? ?????????????????(sprintf ?????stringstream ????
        char buffer[64];
        // ??????? ??????8?????????遺얘턁?????????, ?????????????????????0 ??????????????????????????釉랁닑??????????????醫롫윞??밸븶筌믩끃???醫롫윪?????醫?筌?????????????????醫롫짗壤???
        std::snprintf(buffer, sizeof(buffer), "%.8f", quantity); 
        std::string vol_str(buffer);
        
        LOG_INFO("  ???????袁⑸즴筌????遺얘턁?????????ㅼ굡獒?????????醫롫윞??쇰┛?????醫롫쑌????????????? ??? {:.0f}, ??????{}, ???????醫롫윪?띠슃?????????{:.0f}", 
                 best_ask_price, vol_str, invest_amount);

            // 3. [??????????????????醫롫쑕筌????? ??????????????????醫롫쑕筌???????????醫롫윥??????????????????ш끽維뽳쭩?룸쐻?????醫롫윥?????????????醫롫윪?닳뼹???(OrderManager ????????醫롫윞??밸븶筌믩끃???醫롫윪?????醫?筌????????????
        ...
        if (isLiveRealOrderEnabled()) {
            ...
            // [NEW] OrderManager?????????????????醫롫뱟??????????ш끽維뽳쭩?룸쐻?????醫롫윥?????????????醫롫윪?닳뼹??????????醫롫윪?????????
            // Strategy Metadata ????????醫롫윞??밸븶筌믩끃???醫롫윪?????醫?筌???
            ...
            if (submitted) {
                LOG_INFO("OrderManager ???????袁⑸즴筌????遺얘턁?????????ㅼ굡獒????????븐뼐???????????????ш끽維뽳쭩?룸쐻??醫롫짗??용쐻?? {}", market);
                risk_manager_->reservePendingCapital(invest_amount);  // ???????醫롫윪?????????????????????????
            ...
            // Paper Trading (??????醫롫윥???????????????醫롫윥???????? - ???????????????????釉랁닑????????? (Simulation)
            ...
            // [???????????醫롫짗??????醫롫쑌???????꾩렯????????????????????
            const double fallback_stop_loss_pct = std::clamp(
                config_.foundation_exit_stop_loss_pct,
                0.0,
                0.50
            );
            const double fallback_take_profit_pct = std::clamp(
                config_.foundation_exit_take_profit_pct,
                0.0,
                1.0
            );
            const double fallback_tp1_rr_multiplier = std::clamp(
                config_.foundation_take_profit_1_rr_multiplier,
                0.0,
                2.0
            );
            double dynamic_stop_loss = best_ask_price * (1.0 - fallback_stop_loss_pct);
            try {
                ...
                if (!candles_json.empty() && candles_json.is_array()) {
                    ...
                    if (!candles.empty()) {
                        ...
                        LOG_INFO("[PAPER] ????????醫롫쑕筌????꿔꺂?????諛ㅻ??????????????????醫롫뱟? ???????????? {:.0f} ({:.2f}%)", 
                                 dynamic_stop_loss, (dynamic_stop_loss - best_ask_price) / best_ask_price * 100.0);
                    }
                }
            } catch (const std::exception& e) {
                LOG_WARN("[PAPER] ????????醫롫쑕筌????꿔꺂?????諛ㅻ??????????????????醫롫뱟? ??????????????????????醫롫윥筌?? ??????????-2.5%) ???? {}", e.what());
            }
            ...
            if (signal.stop_loss > 0.0) {
             ...
             const double fallback_tp2 = best_ask_price * (1.0 + fallback_take_profit_pct);
             const double fallback_tp1 = best_ask_price * (1.0 + (fallback_take_profit_pct * fallback_tp1_rr_multiplier));
             double tp1 = signal.take_profit_1 > 0 ? signal.take_profit_1 : fallback_tp1;
             double tp2 = signal.take_profit_2 > 0 ? signal.take_profit_2 : fallback_tp2;
             if (tp2 < tp1) {
                 tp2 = tp1;
             }
        ...
        LOG_ERROR("executeBuyOrder ????????醫롫윪?????? {}", e.what());
        return false;
    }
}
...
void TradingEngine::monitorPositions() {
    ...
    // 1. ????????醫롫윞??밸븶筌믩끃???醫롫윪?????醫?筌????????????醫롫윥獄??????????????醫롫윥??????????醫롫윞??????????????????醫롫윥???????????????醫롫윥?????????醫롫윥?됰쑚????醫롫쑟????筌?????醫롫윪?????????????????????????거??????????遺얘턁?????????醫롫뻿?怨룻돯????醫롫윥?????????
    ...
    // 2. ?????????醫롫윞?????? ??????????醫롫윥??????????醫롫윞????????????????μ떜媛?걫??????????????????????醫롫짗??용쐻??????醫롫쑟???????????醫롫윞??밸븶筌믩끃???醫롫윪?????醫?筌????????醫롫윥???????????????????醫롫윥獒????????(Batch ????????????????????醫롫윞?蹂잙쐻??????????????????????
    ...
    if (should_log) {
        LOG_INFO("===== ??????????醫롫윪?????????????醫롫윪?????????????醫롫윞????({}????????濚밸Ŧ援??? =====", markets.size());
    }

    // 3. [????? ?????????HTTP ?????????????????????醫롫윥???????????????醫롫윥???????????????μ떜媛?걫????????????醫롫윞??밸븶筌믩끃???醫롫윪?????醫?筌????????????????????????醫롫뱟? ?????????????(Batch Processing)
    std::map<std::string, double> price_map;
    
    try {
        // MarketScanner???????????????黎앸럽???醫롫짗????醫롫쑆泳?쉘???getTickerBatch ?????
        ...
        LOG_ERROR("????????????????????????????????????????醫롫윥筌?? {}", e.what());
        return; // ??????????????????????????????醫롫윥甕겸뼹????????????? ??????醫롫윥??????????????? (?????????????醫롫윥???????????????곗뿨?????????????ш끽維뽳쭩?룸쐻??醫롫짗??용쐻?????醫롫윥亦???醫????醫롫윞?蹂잛젂??)
    }

    // 4. ??????醫롫윥?????????????????ш끽維뽳쭩?룸쐻??醫롫짗??용쐻?????????????????????????? ??????????????????????????????????醫롫윥罹?????醫롫윞?????(??????????????????? ??????醫롫윥?????????????????????猷고??燁???
    for (auto& pos : positions) {
        ...
        // RiskManager ???????????????????????醫롫짗????????????썹땟戮녹??諭????醫롫쑕????(??????醫롫윥?????????????????ш끽維뽳쭩?룸쐻??醫롫짗??용쐻????????????耀붾굝???????
        risk_manager_->updatePosition(pos.market, current_price);
        
        // [????????????醫롫짗壤?????????????????嚥싲갭큔?????? ????????醫롫윞??밸븶筌믩끃???醫롫윪?????醫?筌??????"????????????醫롫짗????????????썹땟戮녹??諭????醫롫쑕??????" ??????????????????醫롫윥???????????븐뼐??????????ш끽維??????===================
        std::shared_ptr<strategy::IStrategy> strategy;
        if (strategy_manager_) {
            // ?????????????????醫롫윥獒??????????????????癲ル슢?싩땟?????????醫롫윥筌?짘??????????????醫롫윞??밸븶筌믩끃???醫롫윪?????醫?筌????????醫롫윥??????????????????????(pos.strategy_name ????
            strategy = strategy_manager_->getStrategy(pos.strategy_name);
            
            if (strategy) {
                // ????????????IStrategy????????????醫롫짗壤?????updateState ???????거?????????
                // ???????????醫롫짗??????????????醫롫윞??밸븶筌믩끃???醫롫윪?????醫?筌????????????醫롫윪?????????????????????????????醫롫윪???????耀붾굝????????????????????醫롫짗???????????猷멥럸??ャ걤????????醫롫윥????????????????????????醫롫윥筌?????????????????????醫롫윥??????????????????????
                // ????????븐뼐?????????????????????醫롫윞??밸븶筌믩끃???醫롫윪?????醫?筌????????????醫롫윪????????????????????醫롫짗???????????????????????????醫롫뱟?????????????醫롫윥?????????????????????????醫롫윥?????(??????????????????醫롫쑕筌??????
                strategy->updateState(pos.market, current_price);
            }
        }

        // ??????????????????????????????????醫롫윞??뀀쐻?????????????????????거??????????遺얘턁?????????醫롫뻿?怨룻돯????醫롫윥?????????(?????????醫롫윥獒?????????????????????????????????거????????????
        auto* updated_pos = risk_manager_->getPosition(pos.market);
        if (!updated_pos) continue;
        
        if (should_log) {
            LOG_INFO("  {} - ???????ш끽維뽳쭩?룸쐻??醫롫짗??용쐻?? {} - ?????醫롫윪????????? {:.0f}, ???????ш끽維뽳쭩?룸쐻??醫롫짗??용쐻?? {:.0f}, ????? {:.0f} ({:+.2f}%)",
        ...
        if (strategy) {
            ...
            if (strategy->shouldExit(pos.market, updated_pos->entry_price, current_price, holding_time_seconds)) {
                LOG_INFO("???????ш끽維뽳쭩?룸쐻??醫롫짗??용쐻????????????????????????癲됱빖??????亦껋꼦???????? {} (???????ш끽維뽳쭩?룸쐻??醫롫짗??용쐻?? {})", pos.market, updated_pos->strategy_name);
        ...
        if (!updated_pos->half_closed && current_price >= updated_pos->take_profit_1) {
            LOG_INFO("1?????????????????????????癲됱빖??????亦껋꼦???????? (???????醫롫윥??〓쐻???醫롫짗??????{:+.2f}%)", updated_pos->unrealized_pnl_pct * 100.0);
            executePartialSell(pos.market, *updated_pos, current_price);
            continue; // ???????嚥싲갭큔????????????醫롫윥???????????????곗뿨?????????????????????????????????μ떜媛?걫???????????
        }
        
        // ????????醫롫윞??밸븶筌믩끃???醫롫윪?????醫?筌????????????醫롫윥?????????????(?????or 2???????
        if (risk_manager_->shouldExitPosition(pos.market)) {
            ...
            if (current_price <= updated_pos->stop_loss) {
                ...
                LOG_INFO("???????????????????????癲됱빖??????亦껋꼦????????(???????醫롫윥??〓쐻???醫롫짗??????{:+.2f}%)", updated_pos->unrealized_pnl_pct * 100.0);
            } else if (current_price >= updated_pos->take_profit_2) {
                reason = "take_profit";
                LOG_INFO("2????????????醫롫윪??????????????醫롫윞???醫롫윞???? ??????????????????癲됱빖??????亦껋꼦????????(???????醫롫윥??〓쐻???醫롫짗??????{:+.2f}%)", updated_pos->unrealized_pnl_pct * 100.0);
            } else {
                reason = "strategy_exit"; // ????????醫롫윞??밸븶筌믩끃???醫롫윪?????醫?筌??????????????????????Β?竊???????????醫롫윞??????(TS ??
            }
        ...
        for (const auto& strategy : strategies) {
            ...
            for (const auto& request : order_requests) {
                ...
                if (order_amount < config_.min_order_krw) {
                    LOG_WARN("????????醫롫윥?????????????袁⑸즴筌????遺얘턁?????????ㅼ굡獒?????????醫롫윪?띠슃?????????????????醫롫윪亦????{:.0f} < {:.0f}", order_amount, config_.min_order_krw);
                ...
                if (request.side == autolife::strategy::OrderSide::BUY) {
                    ...
                    if (reserved_capital <= 0.0 || order_amount > reserved_capital) {
                        LOG_WARN("{} ????????醫롫윥???????????醫롫윪??????????????醫롫윪????????醫롫윥??????傭?끆?????醫롫짗????? ????????????????????????醫롫윪亦????({:.0f} < {:.0f})",
...
bool TradingEngine::executeSellOrder(
    ...
    LOG_INFO("???????ш끽維뽳쭩?룸쐻??醫롫짗??용쐻???????醫롫윪????????????釉랁닑?????????????: {} (????: {})", market, reason);
    
    //double current_price = getCurrentPrice(market);
    if (current_price <= 0) {
        LOG_ERROR("???????ш끽維뽳쭩?룸쐻??醫롫짗??용쐻??????????醫롫윪筌??????????醫롫뱟? ???????????????????????醫롫윥筌?? {}", market);
        return false;
    }
    
    // ????????醫롫윞??밸븶筌믩끃???醫롫윪?????醫?筌????????醫롫윥???????????????곗뿨??????
    const double fee_reserve_pct = std::clamp(config_.order_fee_reserve_pct, 0.0, 0.02);
    const double sell_quantity_reserve_pct = std::clamp(fee_reserve_pct * 0.10, 0.0, 0.01);
    const double sell_quantity_multiplier = std::clamp(1.0 - sell_quantity_reserve_pct, 0.90, 1.0);
    const double fill_close_threshold = std::clamp(1.0 - fee_reserve_pct, 0.90, 0.999999);
    double sell_quantity = std::floor(position.quantity * sell_quantity_multiplier * 100000000.0) / 100000000.0;
    ...
    // 1. ??????醫롫윥????????????????????ш끽維뽳쭩?룸쐻?????醫롫윥?????????????醫롫윪?닳뼹????????????ㅼ┥????????????????醫롫윥?????????????(??????醫롫윥?????????????????????醫롫윥???????????? 5,000 KRW)
    if (invest_amount < config_.min_order_krw) {
        LOG_WARN("?????醫롫윪????????????釉랁닑????????????醫롫윪?띠슃?????????????????醫롫윪亦????{:.0f} < {:.0f} (?????醫롫윪???醫롫쑟???????????????醫롫윪???????????", invest_amount, config_.min_order_krw);
        return false;
    }
    
    // 2. ??????????????????醫롫윞????????std::to_string ?????????? ?????????stringstream ????(??????醫롫윥????????????????????醫롫윥??????????醫롫윞???????
        std::stringstream ss;
        ss << std::fixed << std::setprecision(8) << sell_quantity;
        std::string quantity_str = ss.str();

    // 2-1. ?????????醫롫윥????????????????????醫롫윥??????????????????醫롫윥???????????????곗뿨????????? ????????????(??????醫롫윥??????????? ??????醫롫윥???????????????곗뿨?????????????????????
    ...
    try {
        ...
        if (slippage_pct > dynamic_sell_slippage.max_slippage_pct) {
            LOG_WARN("{} ?????? ????? {:.3f}% > {:.3f}% (?????????醫롫윪????????醫롫윥??????傭?끆?????醫롫짗?????",
                     market, slippage_pct * 100.0, dynamic_sell_slippage.max_slippage_pct * 100.0);
            return false;
        }
        LOG_INFO("?????醫롫윪????????????釉랁닑???????? ?????醫롫윪???????????? {} (???????ш끽維뽳쭩?룸쐻??醫롫짗??용쐻??????????醫롫윪筌??????????醫롫뱟?: {})", sell_price, current_price);
    } catch (const std::exception& e) {
        LOG_WARN("??? ???????????????????????醫롫윥筌??(???????ш끽維뽳쭩?룸쐻??醫롫짗??용쐻??????????醫롫윪筌??????????醫롫뱟? ????: {}", e.what());
        sell_price = current_price;
    }

    // 2. ??????????????????醫롫쑕筌?????????????ш끽維뽳쭩?룸쐻?????醫롫윥?????????????醫롫윪?닳뼹??????????? (??????醫롫윥??????????? ????????? ???????ш끽維뽳쭩?룸쐻??醫롫짗??용쐻????醫롫짗????????
    ...
    if (config_.mode == TradingMode::LIVE) {
        if (config_.dry_run || !config_.allow_live_orders) {
            ...
            if (!order_result.success || order_result.executed_volume <= 0.0) {
                LOG_ERROR("?????醫롫윪????????????釉랁닑??????????醫롫윪????????????????????醫롫윥筌?? {}", order_result.error_message);
                return false;
            }

            executed_price = order_result.executed_price;
            execution_order_id = order_result.order_uuid;
            sell_quantity = order_result.executed_volume;  // [Phase 3] ??????????????????醫롫쑕筌???????????醫롫윥???????????????????ш끽維뽳쭩?룸쐻??醫롫짗??용쐻????醫롫짗????????
            
            // [Phase 3] ???????嚥싲갭큔????????????醫롫윥???????????????????????
            double fill_ratio = order_result.executed_volume / position.quantity;
            if (fill_ratio < fill_close_threshold) {
                LOG_WARN("????????醫롫윪亦?????????醫롫윪??????????????????醫롫짗?????: {:.8f}/{:.8f} ({:.1f}%)",
                         order_result.executed_volume, position.quantity, fill_ratio * 100.0);
            }
            
            LOG_INFO("?????醫롫윪????????????釉랁닑??????????醫롫윪??????????????醫롫윥????????????? ??? {:.0f} (?????{})",
    ...
    // 4. [Phase 3] ???????嚥싲갭큔????????????醫롫윥???????????vs ????????醫롫윞??밸븶筌믩끃???醫롫윪?????醫?筌????????醫롫윥?????????????????????????
    double fill_ratio = sell_quantity / position.quantity;
    const bool fully_closed = fill_ratio >= fill_close_threshold;
    if (fully_closed) {
        realized_qty = position.quantity;
        // ????????醫롫윞??밸븶筌믩끃???醫롫윪?????醫?筌????????醫롫윥????????????????????????????????釉랁닑??????(?????????????醫롫윞??밸븶筌믩끃???醫롫윪?????醫?筌??????
    ...
    // 5. [??????????????醫롫윥獒???????? StrategyManager???????????????醫롫윞??밸븶筌믩끃???醫롫윪?????醫?筌?????????醫롫윥???????????????????????????????醫롫짗????????????썹땟戮녹??諭????醫롫쑕????& ?????????????????????????????
    if (fully_closed && strategy_manager_ && !position.strategy_name.empty()) {
        // Position ?????????????????????醫롫윪??롫쐻??醫롫짗????醫롫윪?????????????醫롫윞??귣쐻?????醫롫쑌??????????strategy_name("Advanced Scalping" ??????????????????醫롫윞??밸븶筌믩끃???醫롫윪?????醫?筌????????醫롫윥??????????????????????
...
bool TradingEngine::executePartialSell(const std::string& market, const risk::Position& position, double current_price) {
    ...
    LOG_INFO("  ?????醫롫윪???????????: {:.0f}, ??????????醫롫쑕筌?????: {:.0f}, ????????醫롫윪亦??????????????????????ш끽維뽳쭩?룸쐻??醫롫짗??용쐻?? {:.8f}",
    ...
    try {
        ...
        if (slippage_pct > dynamic_partial_sell_slippage.max_slippage_pct) {
            LOG_WARN("{} ?????? ????? {:.3f}% > {:.3f}% (????????醫롫윪亦?????????????醫롫윪????????醫롫윥??????傭?끆?????醫롫짗?????",
    ...
    if (config_.mode == TradingMode::LIVE) {
        ...
        if (!order_result.success || order_result.executed_volume <= 0.0) {
            LOG_ERROR("????????醫롫윪亦?????????醫롫윪????????????釉랁닑??????????醫롫윪????????????????????醫롫윥筌?? {}", order_result.error_message);
            return false;
        }

        LOG_INFO("????????醫롫윪亦?????????醫롫윪????????????釉랁닑??????????醫롫윪??????????????醫롫윥????????????? ??? {:.0f} (?????{})",
...
double TradingEngine::calculateOptimalBuyPrice(
    ...
    if (units.is_array() && !units.empty()) {
        ...
        return common::roundUpToTickSize(ask);  // [Phase 3] ??? ???????????醫롫윞???????
    }
...
double TradingEngine::calculateOptimalSellPrice(
    ...
    if (units.is_array() && !units.empty()) {
        ...
        return common::roundDownToTickSize(bid);  // [Phase 3] ??? ???????????醫롫윞????????
    }
...
TradingEngine::OrderFillInfo TradingEngine::verifyOrderFill(
    ...
    try {
        ...
        LOG_WARN("???????袁⑸즴筌????遺얘턁?????????ㅼ굡獒???????醫롫윪??????????????醫롫윥???????????????????????醫롫윥筌?? {}", e.what());
    }
...
TradingEngine::LimitOrderResult TradingEngine::executeLimitBuyOrder(
    ...
    while (running_ && remaining > 0.00000001 && attempt_count < max_attempts) {
        ...
        LOG_INFO("?????醫롫윪????????????????袁⑸즴筌????遺얘턁?????????ㅼ굡獒?????????ш끽維뽳쭩?룸쐻??醫롫짗??용쐻????醫롫짗????????醫롫윪筌??(UUID: {}, ????????醫롫짗??????{:.0f}, ?????? {})", uuid, entry_price, vol_str);

        // 10????????????꿔꺂?????⑸솇????????醫롫윥??????????????????거????????????(500ms * 20)
        ...
        // ????????醫롫윪?????????黎앸럽???醫롫짗????醫롫쑆泳?쉘???????猷멥럸??ャ걤???????????????? ????????ш끽維뽳쭩?룸쐻?????醫롫윥?????????????醫롫윪?닳뼹????????????????????
        try {
            http_client_->cancelOrder(uuid);
            LOG_WARN("?????醫롫윪????????????????醫롫윪??????棺堉?뤃??醫롫쑞筌?????猷맣㎫몭???????10?? -> ???????袁⑸즴筌????遺얘턁?????????ㅼ굡獒???????????????????");
        } catch (const std::exception& e) {
            LOG_WARN("?????醫롫윪????????????????袁⑸즴筌????遺얘턁?????????ㅼ굡獒??????????????????醫롫윥筌?? {}", e.what());
        }
...
TradingEngine::LimitOrderResult TradingEngine::executeLimitSellOrder(
    ...
    while (running_ && remaining > 0.00000001 && attempt_count < max_attempts) {
        ...
        std::string uuid = order_res["uuid"].get<std::string>();
        LOG_INFO("?????醫롫윪????????????釉랁닑????????????袁⑸즴筌????遺얘턁?????????ㅼ굡獒?????????ш끽維뽳쭩?룸쐻??醫롫짗??용쐻????醫롫짗????????醫롫윪筌??(UUID: {}, ????????醫롫짗??????{:.0f}, ?????? {})", uuid, exit_price, vol_str);
        ...
        try {
            http_client_->cancelOrder(uuid);
            LOG_WARN("?????醫롫윪????????????釉랁닑????????????醫롫윪??????棺堉?뤃??醫롫쑞筌?????猷맣㎫몭???????10?? -> ???????袁⑸즴筌????遺얘턁?????????ㅼ굡獒???????????????????");
        } catch (const std::exception& e) {
            LOG_WARN("?????醫롫윪????????????釉랁닑????????????袁⑸즴筌????遺얘턁?????????ㅼ굡獒??????????????????醫롫윥筌?? {}", e.what());
        }
...
void TradingEngine::manualScan() {
    LOG_INFO("?????醫롫윥筌띯뮪????????醫롫윥??????醫롫윥????????耀붾굝???????????????");
    scanMarkets();
    generateSignals();
}

// void TradingEngine::manualClosePosition(const std::string& market) {
//     LOG_INFO("??????癲ル슢?싩땟??????????Β?ル윻????嚥싲갭큔?댁빆??????? {}", market);
...
//     LOG_INFO("????????醫롫윞??밸븶筌믩끃???醫롫윪?????醫?筌?????????????癲ル슢?싩땟??????????Β?ル윻????嚥싲갭큔?댁빆???????);
...
double TradingEngine::getCurrentPrice(const std::string& market) {
    try {
        ...
        LOG_ERROR("???????ш끽維뽳쭩?룸쐻??醫롫짗??용쐻??????????醫롫윪筌??????????醫롫뱟? ???????????????????????醫롫윥筌?? {} - {}", market, e.what());
        return 0;
    }
}
...
void TradingEngine::syncAccountState() {
    ...
    try {
        ...
        for (const auto& acc : accounts) {
            ...
            // 1. [?????????醫롫윥獒???????? KRW(???????????? ??????醫롫윥??????????????耀붾굝???????醫롫윥筌????????????????釉랁닑????????????????醫롫짗壤???
            if (currency == "KRW") {
                double total_cash = balance + locked; // ???????????+ ????????醫롫윪?????????黎앸럽???醫롫짗????醫롫쑆泳?쉘???????猷멥럸??ャ걤??????????????遺얘턁???????
                
                // RiskManager??????????????醫롫윥獒??????????????????????????醫롫쑕筌??????????????????????????????????醫롫윥??????
                risk_manager_->resetCapital(total_cash);
                // (2) [????????????醫롫짗壤???] ????????????????????????????????????'????????醫롫쑌????????????????????????????????醫롫쑕筌??????????????????????醫롫윞??????
                config_.initial_capital = total_cash;
                krw_found = true;
                LOG_INFO("???????????????????????????? {:.0f} KRW (????????醫롫짗??????{:.0f})", total_cash, balance);
                continue; // ??????????????????醫롫윥??????????????耀붾굝???????醫롫윥筌?????????????醫롫윪????????????????????醫롫짗壤??????????????????????????
            }

            // 2. ?????????醫롫윞?????? ?????????썹땟戮녹??諭????醫롫쑕???????????醫롫윥??????????????耀붾굝???????醫롫윥筌?????(???????????????????釉랁닑?????????)
            // ??????醫롫윥???????????????????썹땟戮녹??諭????醫롫쑕?????????????袁⑸즴筌?씛彛???돗???????(?? BTC -> KRW-BTC)
            std::string market = "KRW-" + currency;
            wallet_markets.insert(market);
            
            double avg_buy_price = std::stod(acc["avg_buy_price"].get<std::string>());
            
            // ??????醫롫윥???????????븐뼐???????????????Dust) ???????븐뼐??????????(??????醫롫윥?????????????????????醫롫윥????????????????????ш끽維뽳쭩?룸쐻?????醫롫윥?????????????醫롫윪?닳뼹???????????ㅼ┥????????????????????썹땟戮녹??諭????醫롫쑕????
            if (balance * avg_buy_price < 5000) continue;

            // ???? RiskManager?????????????????黎앸럽???醫롫짗??????醫롫윥?????????????怨뚰뇠?????????????
            if (risk_manager_->getPosition(market) != nullptr) continue;

            LOG_INFO("???????????????醫롫윥甕겸뼹????? ???????醫롫윪?룸냵??怨쀬떴??????????袁⑸즴筌?씛彛???돗?????????醫롫윥??년뵾??逾?????? {} (?????? {:.8f}, ???????: {:.0f})", 
                     market, balance, avg_buy_price);

            // [Phase 4] ???????????????????????醫롫짗??용쐻??????醫롫쑟?????????????醫롫윪????????????SL/TP ?????????醫롫윞?????遺얘턁???????????????醫롫윪?????????
            ...
            if (persisted) {
                // [Phase 4] ?????????醫롫윞??귣쐻?????醫롫쑌???????????????????????醫롫윞??밸븶筌믩끃???醫롫윪?????醫?筌????????(?????)
                ...
                LOG_INFO("????????????醫롫윥甕겸뼹????醫롫쑌????????????醫롫쑟??????????醫롫윪?????????: {} SL={:.0f} TP1={:.0f} TP2={:.0f} BE={:.0f} TS={:.0f}",
                         market, safe_stop_loss, tp1, tp2, be_trigger, trail_start);
            } else {
                // ????????????????????????????????????????????????(??????醫롫윥???????????????????? ???????????癲ル슢?싩땟??????????Β?ル윻????嚥싲갭큔?댁빆?????????醫롫윥??????????
                const double fallback_stop_loss_pct = std::clamp(
                    config_.foundation_exit_stop_loss_pct,
                    0.0,
                    0.50
                ...
                double target_sl = avg_buy_price * (1.0 - fallback_stop_loss_pct);
                ...
                tp2 = avg_buy_price * (1.0 + fallback_take_profit_pct);
                tp1 = avg_buy_price * (1.0 + (fallback_take_profit_pct * fallback_tp1_rr_multiplier));
                if (tp2 < tp1) {
                    tp2 = tp1;
                }
                LOG_WARN("????????????醫롫윥甕겸뼹????醫롫쑌????????????醫롫쑟???????????????: {} SL={:.0f} TP1={:.0f} TP2={:.0f} (?????醫롫윪????????????????????????????醫롫윥筌??",
            ...
            // [Phase 4] ???????嚥싲갭큔??????????????????????????????醫롫윞?????遺얘턁????????
            if (half_closed) {
                auto* pos = risk_manager_->getPosition(market);
                if (pos) {
                    pos->half_closed = true;
                    LOG_INFO("  ????????醫롫윪亦???????????????????????????????????醫롫윥甕겸뼹????醫롫쑌??????? {} (half_closed=true)", market);
                }
            }
        }
        
        if (!krw_found) {
            LOG_WARN("???????????????KRW????????醫롫짗???? ???????μ떜媛?걫?????????????醫롫윥筌?짘?? (???????꿔꺂???????????0????????????????醫롫윪??????");
        ...
        if (!pending_reconcile_positions_.empty()) {
            ...
            if (!missing_markets.empty()) {
                try {
                    ...
                    LOG_WARN("???????醫롫윪?????????釉랁닑??????????????醫롫윥???????????????????????????????????????????醫롫윥筌?? {}", e.what());
                }
            }
            ...
            for (const auto& pos : pending_reconcile_positions_) {
                ...
                LOG_INFO("???????醫롫윪?????????釉랁닑??????????????醫롫윥????????????? {} (???????ш끽維뽳쭩?룸쐻??醫롫짗??용쐻?? {}, ????? {:.0f})",
    ...
    try {
        ...
        if (event_journal_) {
            ...
            for (const auto& event : replay_events) {
                // Older snapshots do not carry seq watermark. In that case, replay only newer events by timestamp.
        ...
        LOG_ERROR("??????????????????醫롫윥甕겸뼹????醫롫쑌????????????醫롫쑟???????????????醫롫윥筌?? {}", e.what());
    }
}

// ===== [NEW] ???????????醫롫짗??????醫롫쑌???????꾩렯????????????????????????????醫롫윥???????????????????(?????????醫롫윞????????????????????????????? =====

std::string TradingEngine::exportPrometheusMetrics() const {
    // Prometheus ???????거????????????????醫롫윥筌?????????醫롫윥?????????????耀붾굝?????????????????????????袁⑸즴筌?씛彛???돗???????
    // Grafana?? ???????????醫롫윪??????????醫롫윪????????????????????醫롫윞??뀀쐻??????????醫롫윥???????????????醫롫윥????????????????ㅻ깹??????????醫롫윥??????????
    ...
    // ??????醫롫윥??????????????????(??? ???????耀붾굝????????????????????醫롫짗壤???)
    oss << "# HELP autolife_state AutoLife ??????醫롫윥????????????????????????????????????????거???????????n";
    oss << "# TYPE autolife_state gauge\n";

    // ???????????????醫롫윥獄???????????耀붾굝??????????
    oss << "# HELP autolife_capital_total ???????(KRW)\n";
    oss << "# TYPE autolife_capital_total gauge\n";
    oss << "# HELP autolife_capital_available ?????????????????????????醫롫윞??쎈눀???????????????????????????????????? KRW)\n";
    oss << "# TYPE autolife_capital_available gauge\n";
    oss << "# HELP autolife_capital_invested ??????????????醫롫윥??????????醫롫윞????????????????ㅼ┥????????????????????醫롫윪???????????????????????? KRW)\n";
    oss << "# TYPE autolife_capital_invested gauge\n";

    // ???????????????醫롫윥獄???????????耀붾굝??????????
    oss << "# HELP autolife_pnl_realized ????????????????????醫롫윞??밸븶筌믩끃???醫롫윪?????醫?筌?? KRW)\n";
    oss << "# TYPE autolife_pnl_realized gauge\n";
    oss << "# HELP autolife_pnl_unrealized ????????醫롫윪?????????黎앸럽???醫롫짗????醫롫쑆泳?쉘??????????????????醫롫윞??밸븶筌믩끃???醫롫윪?????醫?筌??????????????, KRW)\n";
    oss << "# TYPE autolife_pnl_unrealized gauge\n";
    oss << "# HELP autolife_pnl_total ??????????????????????醫롫윪?????????黎앸럽???醫롫짗????醫롫쑆泳?쉘????? KRW)\n";
    oss << "# TYPE autolife_pnl_total gauge\n";
    oss << "# HELP autolife_pnl_total_pct ????????醫롫윞??밸븶筌믩끃???醫롫윪?????醫?筌????????????????????醫롫윥獒?????????%)\n";
    oss << "# TYPE autolife_pnl_total_pct gauge\n";

    // ?????????醫롫윥????????關?쒎첎?嫄??醫롫윪???????????????醫롫윥獄???????????耀붾굝??????????
    oss << "# HELP autolife_drawdown_max ??????醫롫윥??????????? ????????醫롫윞??밸븶筌믩끃???醫롫윪?????醫?筌?????????????????????\n";
    oss << "# TYPE autolife_drawdown_max gauge\n";
    oss << "# HELP autolife_drawdown_current ????????醫롫윞??밸븶筌믩끃???醫롫윪?????醫?筌?????????????????????\n";
    oss << "# TYPE autolife_drawdown_current gauge\n";

    // ???????????????醫롫윥獄???????????耀붾굝??????????
    oss << "# HELP autolife_positions_active ????????醫롫윞??밸븶筌믩끃???醫롫윪?????醫?筌???????????醫롫윞?????? ???????n";
    oss << "# TYPE autolife_positions_active gauge\n";
    oss << "# HELP autolife_positions_max ???????????????????醫??룸ㅇ?????????醫롫윥??????????? ???????n";
    oss << "# TYPE autolife_positions_max gauge\n";

    // ??????醫롫윥??????????????????????????耀붾굝??????????
    oss << "# HELP autolife_trades_total ????????醫롫윞??밸븶筌믩끃???醫롫윪?????醫?筌????????醫롫윥????????????????n";
    oss << "# TYPE autolife_trades_total counter\n";
    oss << "# HELP autolife_trades_winning ????????醫롫윞??밸븶筌믩끃???醫롫윪?????醫?筌???????????醫롫윥獒??????????????醫롫윥????????????????n";
    oss << "# TYPE autolife_trades_winning counter\n";
    oss << "# HELP autolife_trades_losing ????????醫롫윞??밸븶筌믩끃???醫롫윪?????醫?筌?????????????醫롫윥????????????????n";
    oss << "# TYPE autolife_trades_losing counter\n";

    // ??????????????????醫롫윥筌?????醫롫짗??????????醫롫윥?????????????????耀붾굝??????????
    oss << "# HELP autolife_winrate ????????醫롫윪?룸냵??怨쀬떴??醫롫쑆????????????0~1)\n";
    oss << "# TYPE autolife_winrate gauge\n";
    oss << "# HELP autolife_profit_factor ?????????醫롫윥獒???????????????Profit Factor)\n";
    oss << "# TYPE autolife_profit_factor gauge\n";
    oss << "# HELP autolife_sharpe_ratio ?????????????醫롫윥????????????????????????????醫롫윥筌?????醫롫짗??????????醫롫윥???????????\n";
    oss << "# TYPE autolife_sharpe_ratio gauge\n";

    // ??????????????????????????耀붾굝??????????
    oss << "# HELP autolife_engine_running ???????????????? ???????????1=??????????0=???)\n";
    oss << "# TYPE autolife_engine_running gauge\n";
    oss << "# HELP autolife_engine_scans_total ?????????????????븐뼐????????????????????醫롫쑌??띕쐻?n";
    oss << "# TYPE autolife_engine_scans_total counter\n";
    oss << "# HELP autolife_engine_signals_total ????????袁⑸즴筌?씛彛???돗????????????????????Β?竊??????????醫롫윥?????????n";
    oss << "# TYPE autolife_engine_signals_total counter\n";

    // ???????????醫롫짗??????醫롫쑌???????꾩렯??????????????????????????????耀붾굝??????????

    // ??????????????醫롫윥????????????????????醫롫윥?????????????耀붾굝???????????????耀붾굝??????????
    oss << "# HELP autolife_buy_orders_total ????????醫롫윞??밸븶筌믩끃???醫롫윪?????醫?筌????????醫롫윥??????????????????ш끽維뽳쭩?룸쐻?????醫롫윥?????????????醫롫윪?닳뼹?????n";
    oss << "# TYPE autolife_buy_orders_total counter\n";
    oss << "# HELP autolife_sell_orders_total ????????醫롫윞??밸븶筌믩끃???醫롫윪?????醫?筌????????醫롫윥???????????????곗뿨??????????????ш끽維뽳쭩?룸쐻?????醫롫윥?????????????醫롫윪?닳뼹?????n";
    oss << "# TYPE autolife_sell_orders_total counter\n";
    oss << "# HELP autolife_pnl_cumulative ????????醫롫윞??밸븶筌믩끃???醫롫윪?????醫?筌????????????????????????????μ떜媛?걫??????????????醫롫윥筌?짘?????? KRW)\n";
...
void TradingEngine::runPrometheusHttpServer(int port) {
    ...
    LOG_INFO("Prometheus ?????醫롫윪????????????轅붽틓????????????븐뼐??????????????????醫롫윞??쇰┛?????醫롫쑌????????????????????ш끽維뽳쭩?룸쐻??醫롫짗??용쐻??(http://localhost:{}/metrics)", port);
