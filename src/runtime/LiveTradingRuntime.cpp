...
double TradingEngine::calculatePositionScaleMultiplier() {
    ...
    // 
    // ???????????熬곣뫖利닷뜝??좎룞?쇿뜝????좎룞?????????????좎럥??????????
    // - WR < 45% || PF < 1.0: 0.5??(?????????????????????????좎룞彛????
    // - 45% <= WR < 50% || 1.0 <= PF < 1.2: 0.75??(?????????좎럡萸?????
    // - 50% <= WR < 60% || 1.2 <= PF < 1.5: 1.0??(???)
    // - WR >= 60% && PF >= 1.5: 1.5??2.5??(???)
    
    auto metrics = risk_manager_->getRiskMetrics();
    
    // ??????좎럥큔??????????????????????????濚밸Ŧ???????????????????????????????? ??????熬곣뫖利닷뜝??좎룞?쇿뜝????좎룞???????????
    ...
    if (win_rate < 0.45 || profit_factor < 1.0) {
        ...
        // ???????????????????????????좎럥痢?????좎룞????????? ?????????????
        // PF?? WR???????????????????????좎럩堉???????????????熬곣뫖利닷뜝??좎룞?쇿뜝????좎룞?????????????좎럥??????????
        // WR 60%~75%: 1.5??2.0?? PF 1.5~2.5: ??????????좎룞彛??? 0.25??
        ...
        new_multiplier = std::min(2.5, new_multiplier);  // ??????좎럥큔?????????? 2.5??
    }
