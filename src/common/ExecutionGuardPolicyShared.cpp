...
double computeHostilityTightenPressure(
    ...
    const double severe_threshold = std::clamp(
        std::max(cfg.hostility_severe_threshold, hostile_threshold + 0.01),
        0.01,
        1.0
    );
...
LiveScanPrefilterThresholds computeLiveScanPrefilterThresholds(
    ...
    const double base_spread = std::clamp(
        cfg.realtime_entry_veto_max_spread_pct * 1.25,
        0.0005,
        0.01
    );
    ...
    const double base_floor = std::max(50000000.0, cfg.min_order_krw * 2000.0);
    double dynamic_min_volume = base_volume * (0.80 + (0.45 * tighten_pressure));
    if (!volumes.empty()) {
        const double median_volume = computeQuantile(volumes, 0.50);
        const double p70_volume = computeQuantile(volumes, 0.70);
        const double universe_anchor =
            median_volume * (0.85 + (0.20 * tighten_pressure));
    ...
    thresholds.max_spread_pct = base_spread * (1.0 - (0.30 * tighten_pressure));
    thresholds.max_spread_pct = std::clamp(thresholds.max_spread_pct, 0.0003, 0.01);

    thresholds.min_ask_notional_krw = std::clamp(
        cfg.min_order_krw * (5.0 + (2.0 * tighten_pressure)),
        cfg.min_order_krw * 3.0,
        cfg.min_order_krw * 12.0
    );
...
RealtimeEntryVetoThresholds computeRealtimeEntryVetoThresholds(
    ...
    if (tighten_pressure <= 1e-9) {
        const double strength = std::clamp((signal_strength - 0.60) / 0.25, 0.0, 1.0);
        const double liquidity = std::clamp((liquidity_score - 58.0) / 20.0, 0.0, 1.0);
        quality_relief = (0.55 * strength) + (0.45 * liquidity);
    }

    if (regime == analytics::MarketRegime::HIGH_VOLATILITY ||
        regime == analytics::MarketRegime::TRENDING_DOWN) {
        tighten_pressure = std::min(1.0, tighten_pressure + 0.12);
    } else if (regime == analytics::MarketRegime::TRENDING_UP &&
               signal_strength >= 0.75 &&
               liquidity_score >= 65.0) {
        quality_relief = std::min(1.0, quality_relief + 0.08);
    }

    thresholds.max_drop_pct *= (1.0 - (0.35 * tighten_pressure) + (0.15 * quality_relief));
    thresholds.max_spread_pct *= (1.0 - (0.40 * tighten_pressure) + (0.18 * quality_relief));
    thresholds.min_orderbook_imbalance += (0.22 * tighten_pressure) - (0.12 * quality_relief);

    thresholds.max_drop_pct = std::clamp(thresholds.max_drop_pct, 0.0004, 0.0200);
    thresholds.max_spread_pct = std::clamp(thresholds.max_spread_pct, 0.0002, 0.0150);
    thresholds.min_orderbook_imbalance =
        std::clamp(thresholds.min_orderbook_imbalance, -0.90, -0.05);
    return thresholds;
}
...
DynamicSlippageThresholds computeDynamicSlippageThresholds(
    ...
    thresholds.max_slippage_pct = std::clamp(cfg.max_slippage_pct, 0.0002, 0.05);
    const double tighten_pressure = computeHostilityTightenPressure(cfg, hostility_ewma);
    ...
    if (is_buy) {
        ...
        if (regime == analytics::MarketRegime::HIGH_VOLATILITY ||
            regime == analytics::MarketRegime::TRENDING_DOWN) {
            tighten = std::min(1.0, tighten + 0.15);
        }
        if (liquidity_score > 0.0 && liquidity_score < 55.0) {
            tighten = std::min(
                1.0,
                tighten + std::clamp((55.0 - liquidity_score) / 25.0, 0.0, 0.25)
            );
        }
        ...
        quality_relief += std::clamp((signal_strength - 0.65) / 0.25, 0.0, 1.0) * 0.20;
        quality_relief += std::clamp((liquidity_score - 60.0) / 20.0, 0.0, 1.0) * 0.15;
        quality_relief += std::clamp((expected_value - 0.0004) / 0.0010, 0.0, 1.0) * 0.12;
        quality_relief *= (1.0 - (0.60 * tighten));

        thresholds.max_slippage_pct *= (1.0 - (0.35 * tighten) + (0.18 * quality_relief));
        thresholds.max_slippage_pct = std::clamp(thresholds.max_slippage_pct, 0.0003, 0.012);
    } else {
        ...
        double relax = 0.15 * tighten_pressure;
        if (urgent_exit) {
            relax += 0.45;
        }
        if (liquidity_score > 0.0 && liquidity_score < 50.0) {
            relax += 0.10;
        }
        if (!urgent_exit &&
