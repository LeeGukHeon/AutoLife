...
private:
    ...
    static constexpr int ORDERBOOK_CACHE_MS = 500;  // 0.5珥?罹먯떆
    
    mutable std::map<std::string, long long> candle_cache_time_;
    mutable std::map<std::string, std::vector<Candle>> candle_cache_;
    static constexpr int CANDLE_CACHE_MS = 2000;    // 2珥?罹먯떆
    
    mutable std::deque<long long> api_call_timestamps_;
    static constexpr int MAX_ORDERBOOK_CALLS_PER_SEC = 8;   // 珥덈떦 8??(?덉쟾 留덉쭊 20%)
    static constexpr int MAX_CANDLE_CALLS_PER_SEC = 8;      // 珥덈떦 8??
    ...
    static constexpr double UPBIT_FEE_RATE = 0.0005;        // 0.05% (怨듭떇)
    static constexpr double EXPECTED_SLIPPAGE = 0.0003;     // 0.03%
    static constexpr double MIN_ORDER_AMOUNT_KRW = 5000.0;  // 5,000??(嫄곕옒??理쒖냼)
    ...
    static constexpr double MAX_HOLDING_TIME = 1800.0;      // 30遺?
    ...
    static constexpr int MIN_SIGNAL_INTERVAL_SEC = 120;     // 2遺?
