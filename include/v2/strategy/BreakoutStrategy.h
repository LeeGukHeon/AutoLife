...
enum class BreakoutType {
    NONE,
    DONCHIAN_BREAK,       // ?꾩튂??梨꾨꼸 ?뚰뙆
    RESISTANCE_BREAK,     // ???꽑 ?뚰뙆
    CONSOLIDATION_BREAK,  // ?〓낫 ?댄깉
    VOLUME_BREAKOUT       // 嫄곕옒??湲됱쬆 ?뚰뙆
};
...
private:
    ...
    // ?ъ???異붿쟻 (?닿굔 ?대? 濡쒖쭅?⑹씠怨? 以묐났 諛⑹???active_positions_媛 ?대떦)
    std::map<std::string, BreakoutPositionData> position_data_; // active_positions_? ?대쫫 寃뱀튂吏 ?딄쾶 二쇱쓽
    ...
    static constexpr double MAX_HOLDING_TIME_MINUTES = 300.0;  // 5?쒓컙
    ...
    static constexpr int MIN_SIGNAL_INTERVAL_SEC = 720;     // 12遺?
